...
void RankManager::OnDayChange() //现在一个小时更新
{
...
void RankManager::OnHourChange() //现在一个小时更新
{
	OnLoadRankInfoByDB();//发送命令加载当天的数据 排行榜数据
}
...
void RankManager::SaveNowDayRankInfo()
{
	...
	char FileName[32] = { 0 };
	sprintf_s(FileName, CountArray(FileName), "Rank_%d_%d_%d_%d.xml", time.wYear, time.wMonth, time.wDay,time.wHour);
#else
	char FileName[32] = { 0 };
	sprintf_s(FileName, CountArray(FileName), "Rank.xml");
#endif
	...
	for (; Iter != m_RankArray.end(); ++Iter)
	{
		...
		for (int i = 1; IterVec != Iter->second.end(); ++IterVec, ++i)
		{
			...
			Info->SetAttribute("VipLevel", IterVec->VipLevel);
			char Int64[64] = { 0 };
