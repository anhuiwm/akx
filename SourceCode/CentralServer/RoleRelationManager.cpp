...
void RoleRelationManager::OnChangeRelationUserWeekGloabl(DWORD dwUserID, __int64 WeekGoldNum)
{
	if (!m_pRole)
	{
		ASSERT(false);
	...
	if (Iter != m_RelationMap.end())
	{
		CC_Cmd_ChangeRelationWeekGlobal msg;
		SetMsgInfo(msg, GetMsgType(Main_Relation, CC_ChangeRelationWeekGlobal), sizeof(CC_Cmd_ChangeRelationWeekGlobal));
		msg.dwUserID = m_pRole->GetRoleID();
		msg.dwDestUserID = dwUserID;
		msg.WeekGlodNum = WeekGoldNum;
		m_pRole->SendDataToGameServer(&msg);
		return;
	}
	else
