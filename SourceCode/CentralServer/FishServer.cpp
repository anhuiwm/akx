...
bool FishServer::HandleGameServerMsg(ServerClientData* pClient, NetCmd* pCmd)
{
	...
	else if (pCmd->CmdType == Main_Role)
	{
		...
		case CC_ChangeRoleWeekGlobe:
		{
			CC_Cmd_ChangeRoleWeekGlobal* pMsg = CHECKMSG(pCmd, pCmd->CmdSize, CC_Cmd_ChangeRoleWeekGlobal);
			if (!pMsg)
			{
				ASSERT(false);
				return false;
			...
			pRole->OnChangeRoleWeekGloabl(pMsg->WeekGoldNum);
			return true;
		}
		...
		case OC_UseRMB:
			{
				...
				else
				{
					...
					{
						//添加一个新的命令 保存玩家离线充值的数据 DBR_Cmd_AddRecharg
...
bool FishServer::HandleDataBaseMsg(NetCmd* pCmd)
{
	...
	case DBO_AddUserMail:
		...
		return true;
	case DBO_AddAllUserMail:
		HandleAddAllUserMailResult(pCmd);
		return true;
...
bool FishServer::HandleAddUserMailResult(NetCmd* pCmd)
{
	...
	if (pMsg->Result && pMsg->dwDestUserID != 0)
	{
...
bool FishServer::HandleAddAllUserMailResult(NetCmd* pCmd)
{
	DBO_Cmd_AddUserMail* pMsg = (DBO_Cmd_AddUserMail*)pCmd;
	if (!pMsg)
	{
	...
	if (pMsg->Result)
	{
		m_RoleManager.SendAllUserMail([&pMsg] (CenterRole* pRole)
		{
			if (pRole)
			{
				//玩家在线 直接发到玩家去
				CG_Cmd_SendUserMail msg;
				SetMsgInfo(msg, GetMsgType(Main_Mail, CG_SendUserMail), sizeof(CG_Cmd_SendUserMail));
				msg.DestUserID = pRole->GetRoleID();
				msg.MailInfo = pMsg->MailInfo;
				pRole->SendDataToGameServer(&msg);
			}
		});
	...
	case CL_SendSystemEmail://外部发送系统邮件
		{
			...
			if (pMsg->dwUserID == 0)
			{
				...
				MailInfo.bIsExistsReward = (MailInfo.RewardID != 0 && MailInfo.RewardSum != 0);
				DBR_Cmd_AddUserMail msg;
				SetMsgInfo(msg, DBR_AddAllUserMail, sizeof(DBR_Cmd_AddUserMail));
				msg.dwDestUserID = 0;
				msg.MailInfo = MailInfo;
				g_FishServer.SendNetCmdToDB(&msg);
			}
