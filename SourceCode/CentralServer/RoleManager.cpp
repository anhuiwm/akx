...
void CenterRoleManager::SendAllUserMail(std::function<void(CenterRole* pRole)> func)
{
	if (m_RoleMap.empty())
		return;
	HashMap<DWORD, CenterRole*>::iterator Iter = m_RoleMap.begin();
...
void CenterRole::OnChangeRoleWeekGloabl(__int64 WeekGoldNum)
{
	m_RoleInfo.WeekGoldNum = WeekGoldNum;
	HashMap<DWORD, tagBeRoleRelation>& pMap = GetRelationManager().GetAllBeRelationInfo();
	HashMap<DWORD, tagBeRoleRelation>::iterator Iter = pMap.begin();
	for (; Iter != pMap.end(); ++Iter)
	{
		...
		if (pRole)
		{
			pRole->GetRelationManager().OnChangeRelationUserWeekGloabl(m_RoleInfo.dwUserID, WeekGoldNum);
		}
	}
}
