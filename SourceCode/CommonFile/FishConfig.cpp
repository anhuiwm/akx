...
void FishConfig::ShowInfoToWin(const char *pcStr, ...)
{
	if (!pcStr)
	{
		ASSERT(false);
...
bool FishConfig::LoadConfigFilePath()
{
	...
	{
		ShowInfoToWin("FishConfig.xml解析失败");
		//ASSERT(false);
		return false;
	}
	...
	if (!pFishConfig)
	{
		ShowInfoToWin("FishConfig.xml::FishConfig节点错误");
		//ASSERT(false);
		return false;
	}
	//读取物品
	...
	{
		ShowInfoToWin("FishConfig.xml::FishItems节点错误");
		//ASSERT(false);
		return false;
	}

	//物品类型 
	if (!LoadFishItemTypeConfig(pFishConfig))
	{
		ShowInfoToWin("FishConfig.xml::FishItemTypes节点错误");
		//ASSERT(false);
		return false;
	}

	//合成
	if (!LoadFishComposeConfig(pFishConfig))
	{
		ShowInfoToWin("FishConfig.xml::Composes节点错误");
		//ASSERT(false);
		return false;
	}
	...
	{
		ShowInfoToWin("FishConfig.xml::FishRewardInfo节点错误");
		//ASSERT(false);
		return false;
	}
	//1.Check
	...
	{
		ShowInfoToWin("FishConfig.xml::FishCheck节点错误");
		//ASSERT(false);
		return false;
	}
	//2.Task
	...
	{
		ShowInfoToWin("FishConfig.xml::FishTask节点错误");
		//ASSERT(false);
		return false;
	}
	//成就
	...
	{
		ShowInfoToWin("FishConfig.xml::FishAchievements节点错误");
		//ASSERT(false);
		return false;
	}
	//3.Month
	...
	{
		ShowInfoToWin("FishConfig.xml::FishMonths节点错误");
		//ASSERT(false);
		return false;
	}
	//4.称号
	...
	{
		ShowInfoToWin("FishConfig.xml::FishTitles节点错误");
		//ASSERT(false);
		return false;
	}
	//5.排行榜
	...
	{
		ShowInfoToWin("FishConfig.xml::FishRanks节点错误");
		//ASSERT(false);
		return false;
	}
	//6.宝箱
	...
	{
		ShowInfoToWin("FishConfig.xml::FishChests节点错误");
		//ASSERT(false);
		return false;
	}
	//7.魅力
	...
	{
		ShowInfoToWin("FishConfig.xml::FishCharms节点错误");
		//ASSERT(false);
		return false;
	}
	//8.商店
	...
	{
		ShowInfoToWin("FishConfig.xml::FishShops节点错误");
		//ASSERT(false);
		return false;
	}
	//9.活动
	...
	{
		ShowInfoToWin("FishConfig.xml::FishActions节点错误");
		//ASSERT(false);
		return false;
	}
	//10.赠送
	...
	{
		ShowInfoToWin("FishConfig.xml::FishGiff节点错误");
		//ASSERT(false);
		return false;
	}
	//11.桌子参数
	...
	{
		ShowInfoToWin("FishConfig.xml::FishTables节点错误");
		//ASSERT(false);
		return false;
	}
	//12.关系配置
	...
	{
		ShowInfoToWin("FishConfig.xml::FishRelations节点错误");
		//ASSERT(false);
		return false;
	}
	//13.物品转换
	...
	{
		ShowInfoToWin("FishConfig.xml::FishServerUpdateTime节点错误");
		//ASSERT(false);
		return false;
	}
	//服务器更新时间
	...
	{
		ShowInfoToWin("FishConfig.xml::FishMail节点错误");
		//ASSERT(false);
		return false;
	}
	//金币商店
	...
	{
		ShowInfoToWin("FishConfig.xml::fishonlinereward节点错误");
		//ASSERT(false);
		return false;
	}
	//礼包
	...
	{
		ShowInfoToWin("FishConfig.xml::fishpackage节点错误");
		//ASSERT(false);
		return false;
	}
	if (!LoadFishSystemConfig(pFishConfig))
	{
		ShowInfoToWin("FishConfig.xml::fishsystem节点错误");
		//ASSERT(false);
		return false;
	}
	if (!LoadFishRechargeConfig(pFishConfig))//加载RMB充值配置
	{
		ShowInfoToWin("FishConfig.xml::FishRecharges节点错误");
		//ASSERT(false);
		return false;
	}
	if (!LoadFishLevelReward(pFishConfig))
	{
		ShowInfoToWin("FishConfig.xml::levelreward节点错误");
		return false;
	}
	if (!LoadFishExChangeConfig(pFishConfig))
	{
		ShowInfoToWin("FishConfig.xml::exchange节点错误");
		return false;
	}
	if (!LoadFishChannelConfig(pFishConfig))
	{
		ShowInfoToWin("FishConfig.xml::channel节点错误");
		return false;
	}
	if (!LoadFishRoleProtectConfig(pFishConfig))
	{
		ShowInfoToWin("FishConfig.xml::protect节点错误");
		return false;
	}
	if (!LoadFishLotteryConfig(pFishConfig))
	{
		ShowInfoToWin("FishConfig.xml::lottery节点错误");
		return false;
	}
	if (!LoadFishMonthCardConfig(pFishConfig))
	{
		ShowInfoToWin("FishConfig.xml::monthcard节点错误");
		return false;
	}
	if (!LoadFishVipConfig(pFishConfig))
	{
		ShowInfoToWin("FishConfig.xml::vip节点错误");
		return false;
	}
	if (!LoadFishDropConfig(pFishConfig))
	{
		ShowInfoToWin("FishConfig.xml::drop节点错误");
		return false;
	}
	if (!LoadFishMiniGameConfig(pFishConfig))
	{
		ShowInfoToWin("FishConfig.xml::minigame节点错误");
		return false;
	}
	if (!LoadFishGameRobotConfig(pFishConfig))
	{
		ShowInfoToWin("FishConfig.xml::robot节点错误");
		return false;
	}
	//过滤关键字
	if (!LoadFishErrorStringFile("ErrorString.txt"))
	{
		ShowInfoToWin("FishConfig.xml::errorstring节点错误");
		return false;
	}
	...
	if (!LoadFishBroadCastConfig(TEXT("FishNoticeConfig.xml")))//加载公告的数据
	{
		ShowInfoToWin("FishNoticeConfig1节点错误");
		return false;
	}

	if (!LoadFishOpenGameIDConfig(TEXT("FishNoticeConfig.xml")))
	{
		ShowInfoToWin("FishNoticeConfig2节点错误");
		return false;
	}

	if (!LoadFishOpenClientIPConfig(TEXT("FishNoticeConfig.xml")))
	{
		ShowInfoToWin("FishNoticeConfig3节点错误");
		return false;
	}
	
	if (!LoadRobotConfig(TEXT("robotconfig.xml")))//
	{
		ShowInfoToWin("robotconfig点错误");
		return false;
	}
	return true;
}
...
bool FishConfig::LoadFishTask(WHXmlNode* pFishConfig)
{
	...
	//if (!pTasks->GetAttribute(TEXT("MaxDayTaskID"), m_TaskConfig.m_MaxDayTaskID))
	//{
	//	ASSERT(false);
	//	return false;
	//}
	...
	while (pTask)
	{
		...
		//BYTE IsClear = 0;
		if (!pTask->GetAttribute(TEXT("Activeness"), pInfo.Activeness))
		{
			return false;
		}

		if (!pTask->GetAttribute(TEXT("Sign"), pInfo.Sign))
		{
			return false;
		}
		if (pInfo.Sign <= MIN_TASK || pInfo.Sign >= MAX_TASK)
		{
			ASSERT(false);
			return false;
		}
		...
		if (pInfo.Sign != 0)
		{
			HashMap<BYTE, std::vector<BYTE>>::iterator Iter = m_TaskConfig.m_TaskSignMems.find(pInfo.Sign);
			if (Iter == m_TaskConfig.m_TaskSignMems.end())
			{
				std::vector<BYTE> pVec;
				pVec.push_back(pInfo.TaskID);
				m_TaskConfig.m_TaskSignMems.insert(HashMap<BYTE, std::vector<BYTE>>::value_type(pInfo.Sign, pVec));
			}
			else
			{
				Iter->second.push_back(pInfo.TaskID);
...
bool FishConfig::LoadFishShop(WHXmlNode* pFishConfig)
{
	...
	while (pShop)
	{
		...
		while (pItem)
		{
			...
			//TCHAR name[32] = { 0 };
			if (!pItem->GetAttributeUtf8(TEXT("ItemName"), pItemConfig.ShopItemName, CountArray(pItemConfig.ShopItemName)))
				return false;
			//TCHARCopy(name, CountArray(name), pItemConfig.ShopItemName, _tcslen(pItemConfig.ShopItemName));
...
bool FishConfig::LoadFishAction(WHXmlNode* pFishConfig)
{
	...
	while (pFishAction)
	{
		...
		if (!pFishAction->GetAttribute(TEXT("Sign"), pAction.Sign))
		{
			return false;
		}
...
bool FishConfig::LoadFishItemConfig(WHXmlNode* pFishConfig)
{
	...
	while (pFishItem)
	{
		...
		if (!pFishItem->GetAttribute(TEXT("Attribute"), pItemConfig.Attribute))
		{
			pItemConfig.Attribute = 0;
		}
...
bool FishConfig::LoadFishItemTypeConfig(WHXmlNode* pFishConfig)
{
	m_ItemTypeMap.clear();
	WHXmlNode* pFishItems = pFishConfig->GetChildNodeByName(TEXT("FishItemTypes"));
	if (!pFishItems)
	{
		ASSERT(false);
		return false;
	...
	while (pFishItem)
	{
		tagSendItemTypeConfig pItemConfig;

		if (!pFishItem->GetAttribute(TEXT("TypeID"), pItemConfig.TypeID))
		{
			return false;
		}
		int CanSend = 0;
		if (!pFishItem->GetAttribute(TEXT("CanSend"), CanSend))
		{
			//
		}
		pItemConfig.CanType = BYTE(CanSend);

		m_ItemTypeMap.insert(HashMap<BYTE, tagSendItemTypeConfig>::value_type(pItemConfig.TypeID, pItemConfig));
		pFishItem = pFishItem->GetNextSignelNode();
	}
	return true;
}
...
bool FishConfig::LoadFishComposeConfig(WHXmlNode* pFishConfig)
{
	m_ComposeMap.clear();
	WHXmlNode* pFishItems = pFishConfig->GetChildNodeByName(TEXT("Composes"));
	if (!pFishItems)
	{
		ASSERT(false);
		return false;
	...
	while (pFishItem)
	{
		tagComposeConfig pItemConfig;

		if (!pFishItem->GetAttribute(TEXT("ItemID"), pItemConfig.ItemID))
		{
			return false;
		}
		if (!pFishItem->GetAttribute(TEXT("ItemParam"), pItemConfig.ItemNum))
		{
			return false;
		}

		if (!pFishItem->GetAttribute(TEXT("ComposeID"), pItemConfig.ComposeID))
		{
			return false;
		}

		m_ComposeMap.insert(HashMap<DWORD, tagComposeConfig>::value_type(pItemConfig.ItemID, pItemConfig));
		pFishItem = pFishItem->GetNextSignelNode();
	}
	return true;
}
...
bool FishConfig::LoadFishOnlineRewardConfig(WHXmlNode* pFishConfig)
{
	...
	while (pOnce)
	{
		...
		ZeroMemory(&pOnceReward, sizeof(pOnceReward));

		if (!pOnce->GetAttribute(TEXT("ID"), pOnceReward.ID))
			return false;
		if (pOnceReward.ID < 1 || pOnceReward.ID>32)//ID 范围为1到32
			return false;
		if (!pOnce->GetAttribute(TEXT("OnlineMin"), pOnceReward.OnlineSec) && pOnceReward.ID < 20)
		{
			//return false;
		}
		if (!pOnce->GetAttribute(TEXT("RewardID"), pOnceReward.RewardID))
			return false;
		if (!pOnce->GetAttribute(TEXT("OnlineStartHour"), pOnceReward.OnlineStartHour) && pOnceReward.ID >= 20)
		{
			//return false;
		}
		if (!pOnce->GetAttribute(TEXT("OnlineEndHour"), pOnceReward.OnlineEndHour) && pOnceReward.ID >= 20)
		{
			//return false;
		}
		pOnceReward.OnlineSec = pOnceReward.OnlineSec * 60;//配置为分钟
...
bool FishConfig::LoadFishSystemConfig(WHXmlNode* pFishConfig)
{
	...
	{
		m_SystemConfig.MaxGobelSum = 900000000000;
	}
	...
	if (!pFishSystem->GetAttribute(TEXT("EmailCurrceyRewardID"), m_SystemConfig.EmailCurrceyRewardID))
		return false;
	if (!pFishSystem->GetAttribute(TEXT("EmailCashpointRewardID"), m_SystemConfig.EmailCashpointRewardID))
		return false;
	...
	if (!pFishSystem->GetAttribute(TEXT("SendMessageItemSum"), m_SystemConfig.SendMessageItemSum))
		return false;
	if (!pFishSystem->GetAttribute(TEXT("MaxSendRecvItemRecord"), m_SystemConfig.MaxSendRecvItemRecord))
		return false;
	if (!pFishSystem->GetAttribute(TEXT("GoldBulletItemID"), m_SystemConfig.GoldBulletItemID))
		return false;
	if (!pFishSystem->GetAttribute(TEXT("NewFishRewardID"), m_SystemConfig.NewFishRewardID))
		return false;
	if (!pFishSystem->GetAttribute(TEXT("ServerState"), m_SystemConfig.ServerState))
		return false;
	if (!pFishSystem->GetAttribute(TEXT("LogonServerCheck"), m_SystemConfig.LogonServerCheck))
		return false;
	return true;
}
...
bool FishConfig::LoadFishChannelConfig(WHXmlNode* pFishConfig)
{
	WHXmlNode* pFishExChanges = pFishConfig->GetChildNodeByName(TEXT("FishChannels"));
	if (!pFishExChanges)
	{
		ASSERT(false);
		return false;
	}
	WHXmlNode* pExChange = pFishExChanges->GetChildNodeByName(TEXT("FishChannel"));
	while (pExChange)
	{
		TCHAR sdk[64];
		if (!pExChange->GetAttribute(TEXT("sdk"), sdk, CountArray(sdk)))
			return false;
		TCHAR name[64];
		if (!pExChange->GetAttribute(TEXT("name"), name, CountArray(name)))
			return false;

		m_ChannelMap.insert(make_pair(sdk,name));
...
bool FishConfig::LoadFishVipConfig(WHXmlNode* pFishConfig)
{
	...
	pTotalOnce.SendGoldBulletNum   = 0;
	pTotalOnce.SendSilverBulletNum = 0;
	...
	while (pFishVip)
	{
		...
		if (!pFishVip->GetAttribute(TEXT("AddLauncherID"), AddLauncherID))
			return false;
		if (!pFishVip->GetAttribute(TEXT("SendGoldBulletNum"), pCurrceyOnce.SendGoldBulletNum))
			return false;
		if (!pFishVip->GetAttribute(TEXT("SendSilverBulletNum"), pCurrceyOnce.SendSilverBulletNum))
			return false;
		if (!pFishVip->GetAttribute(TEXT("SendBronzeBulletNum"), pCurrceyOnce.SendBronzeBulletNum))
			return false;
		if (!pFishVip->GetAttribute(TEXT("PackgeID"), pCurrceyOnce.RewardID))
			return false;
		if (!pFishVip->GetAttribute(TEXT("SendItemNum"), pCurrceyOnce.SendItemNum))
			return false;
		if (!pFishVip->GetAttribute(TEXT("LoginRewardID"), pCurrceyOnce.LoginRewardID))
			return false;
		...
		pTotalOnce.SendGoldBulletNum = pCurrceyOnce.SendGoldBulletNum;
		pTotalOnce.SendSilverBulletNum = pCurrceyOnce.SendSilverBulletNum;
		pTotalOnce.SendBronzeBulletNum = pCurrceyOnce.SendBronzeBulletNum;
		pTotalOnce.RewardID =  pCurrceyOnce.RewardID;
		pTotalOnce.SendItemNum = pCurrceyOnce.SendItemNum;
		pTotalOnce.LoginRewardID = pCurrceyOnce.LoginRewardID;
...
bool FishConfig::LoadFishDropConfig(WHXmlNode* pFishConfig)
{
	...
	//std::map<BYTE, std::map<BYTE, tagFishDropOnce> > FishDropMap;

	WHXmlNode*  pType = pFishDrop->GetChildNodeByName(TEXT("type"));
	while (pType)
	{
		BYTE FishTypeID;
		if (!pType->GetAttribute(TEXT("ID"), FishTypeID))
			return false;

		std::map<BYTE, tagFishDropOnce> rateDropMap;
		WHXmlNode*  pRate = pType->GetChildNodeByName(TEXT("RateID"));
		while (pRate)
		{
			BYTE MaxRateID;
			if (!pRate->GetAttribute(TEXT("Max"), MaxRateID))
				return false;

			tagFishDropOnce pFishOnce;
			pFishOnce.FishTypeID = FishTypeID;
			pFishOnce.TotalRateValue = 0;
			WHXmlNode*  pOnce = pRate->GetChildNodeByName(TEXT("Once"));
			while (pOnce)
			{
				DropInfo pDrop;
			...
			rateDropMap.insert(std::make_pair(MaxRateID, pFishOnce));
			pRate = pRate->GetNextSignelNode();
		}
		//FishDropMap.insert(HashMap<BYTE, std::map<BYTE, tagFishDropOnce> >::value_type(FishTypeID, rateDropMap));
		m_FishDropConfig.FishDropMap.insert(HashMap<BYTE, std::map<BYTE, tagFishDropOnce> >::value_type(FishTypeID, rateDropMap));
...
BYTE FishConfig::IsTimeItem(DWORD ItemID)
{
	tagItemConfig* pConfig = GetItemInfo(ItemID);
	if (!pConfig)
		return 0;
	else
		return pConfig->IsTimeType();
}
...
tagSendItemTypeConfig* FishConfig::GetItemTypeInfo(BYTE type)
{
	HashMap<BYTE, tagSendItemTypeConfig>::iterator Iter = m_ItemTypeMap.find(type);
	if (Iter == m_ItemTypeMap.end())
		return NULL;
	else
		return &Iter->second;
}

tagComposeConfig* FishConfig::GetComposeInfo(DWORD ItemID)
{
	HashMap<DWORD, tagComposeConfig>::iterator Iter = m_ComposeMap.find(ItemID);
...
bool FishConfig::CanSendItemType(BYTE type)
{
	tagSendItemTypeConfig* pcfg = FishConfig::GetItemTypeInfo(type);
	if (pcfg != NULL && pcfg->CanType>0)
	{
		return true;
	}
	return false;
...
bool FishConfig::IsGmGameID(DWORD gameid)
{
	HashMap<DWORD, WORD>::iterator Iter = m_OpenLevelIDMap.m_OpenLevelIDMap.find(gameid);
	if (Iter != m_OpenLevelIDMap.m_OpenLevelIDMap.end())
	{
		return true;// = Iter->second;//1为测试账户
	}
	return false;
}

bool FishConfig::IsGmClientIP(DWORD clientip)
{
	HashMap<DWORD, WORD>::iterator Iter = m_OpenLevelIPMap.m_OpenLevelIPMap.find(clientip);
	if (Iter != m_OpenLevelIPMap.m_OpenLevelIPMap.end())
	{
		return true;
	}
	return false;
...
//OpenLevel开放等级 0为没有特权  1为维护可登录游戏   2为直接给vip9
bool FishConfig::IsLogonGm(DWORD gameid, DWORD clientip)
{
	if (gameid != 0)
	{
		HashMap<DWORD, WORD>::iterator Iter = m_OpenLevelIDMap.m_OpenLevelIDMap.find(gameid);
...
bool FishConfig::IsChargeGm(DWORD gameid, DWORD clientip)
{
	if (gameid != 0)
	{
		HashMap<DWORD, WORD>::iterator Iter = m_OpenLevelIDMap.m_OpenLevelIDMap.find(gameid);
...
bool FishConfig::CheckServerState()
{
	//0为未开放  1为开放  2为维护中
	if (GetSystemConfig().ServerState != 1)
	{
		return false;
	}
	return true;
...
bool FishConfig::LogonServerCheck()
{
	//1为LogonServerCheck
	if (GetSystemConfig().LogonServerCheck >= 1)
	{
		return true;
	}
	return false;
...
bool FishConfig::LoadFishBroadCastConfig(const TCHAR* FilePath)
{
	WHXml pXml;
	if (!pXml.LoadXMLFilePath(FilePath))
	{
	...
	WHXmlNode* pNotices = pXml.GetChildNodeByName(TEXT("Broadcasts"));
	if (!pNotices)
	{
		ASSERT(false);
		return false;
	...
	m_NoticeMap.clear();
	WHXmlNode* pNotice = pNotices->GetChildNodeByName(TEXT("Broadcast"));
	while (pNotice)
	{
		tagNotice pNoticeOnce;
		//单条公告的读取
		...
		pNoticeOnce.BeginTime = mktime(&pTime);
		//
		if (!pNotice->GetAttribute(TEXT("Type"), pNoticeOnce.ID))
			return false;
		if (!pNotice->GetAttribute(TEXT("StepNum"), pNoticeOnce.StepNum))
			return false;
		if (!pNotice->GetAttribute(TEXT("StepMin"), pNoticeOnce.StepMin))
		...
		if (!pNotice->GetAttribute(TEXT("MessageInfoColor"), pNoticeOnce.MessageInfoColor))
			return false;
		m_BroadCastMap.insert(HashMap<DWORD, tagNotice>::value_type(pNoticeOnce.ID, pNoticeOnce));
...
bool FishConfig::LoadFishOpenGameIDConfig(const TCHAR* FilePath)
{
	WHXml pXml;
	if (!pXml.LoadXMLFilePath(FilePath))
	{
	...
	WHXmlNode* pFishLevelReward = pXml.GetChildNodeByName(TEXT("FishOpenID"));
	if (!pFishLevelReward)
	{
		ASSERT(false);
		return false;
	}
	WHXmlNode* pGameIDLevel = pFishLevelReward->GetChildNodeByName(TEXT("OpenLevel"));
	while (pGameIDLevel)
	{
		DWORD ID = 0;
		WORD Level = 0;

		if (!pGameIDLevel->GetAttribute(TEXT("UserID"), ID))
			return false;
		if (!pGameIDLevel->GetAttribute(TEXT("OpenLevel"), Level))
			return false;
		m_OpenLevelIDMap.m_OpenLevelIDMap.insert(HashMap<DWORD, WORD>::value_type(ID, Level));
		pGameIDLevel = pGameIDLevel->GetNextSignelNode();
	}
	return true;
}

bool FishConfig::LoadFishOpenClientIPConfig(const TCHAR* FilePath)
{
	WHXml pXml;
	if (!pXml.LoadXMLFilePath(FilePath))
	{
	...
	WHXmlNode* pFishLevelReward = pXml.GetChildNodeByName(TEXT("FishOpenIP"));
	if (!pFishLevelReward)
	{
		ASSERT(false);
		return false;
	}
	WHXmlNode* pGameIDLevel = pFishLevelReward->GetChildNodeByName(TEXT("OpenIPLevel"));
	while (pGameIDLevel)
	{
		DWORD ID = 0;
		WORD Level = 0;
		...
		if (!pGameIDLevel->GetAttribute(TEXT("ClientIP"), ClientIP, IP_MAX_LENGTH))
			return false;
		UINT ClientCount = 0;
		...
		inet_pton(AF_INET, pClientChar, (void *)&ID);
		free(pClientChar);
		if (!pGameIDLevel->GetAttribute(TEXT("OpenLevel"), Level))
			return false;
		m_OpenLevelIPMap.m_OpenLevelIPMap.insert(HashMap<DWORD, WORD>::value_type(ID, Level));
		pGameIDLevel = pGameIDLevel->GetNextSignelNode();
	}
	return true;
}
