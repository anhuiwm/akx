...
//sdk  String  是  09CE2B99C22E6D06  渠道在易接服务器上的ID。由易接提供。      channlid
//app  String  是  800018D72E2761D0  CP游戏在易接服务器上的ID。由易接提供。    productcode
//uin  String  是  123456  在渠道上的用户ID。                                  channluid
//sess  String  是  abcdefghijklmn  用户在登陆渠道后，由渠道返回给客户端的SessionId。  token
//
	UINT Size = 0;
...
bool FishServer::OnHandleGetRoleInfoByUserID(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	if (m_Sql[Index].Select(SqlStr, 0, pTable1, true) && pTable1.Rows() == 1)
	{
		...
		msg->RoleInfo.dwGoldBulletNum = pTable1.GetUint(0, 54);;//黄金弹头数量
		msg->RoleInfo.byUsingLauncher = pTable1.GetByte(0, 55);;//当前使用的炮台
		...
		msg->RoleInfo.SendGoldBulletNum = pTable1.GetUint(0, 62);;//黄金弹头数量
		msg->RoleInfo.SendSilverBulletNum = pTable1.GetUint(0, 63);;//黄金弹头数量
		msg->RoleInfo.SendBronzeBulletNum = pTable1.GetUint(0, 64);;//黄金弹头数量
		msg->RoleInfo.GuideStep = pTable1.GetUint(0, 65);;//黄金弹头数量
...
bool FishServer::OnHandleChangeUserItem(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	char SqlStr[MAXSQL_LENGTH] = { 0 };
	//if (pMsg->EndTime > 0)
	if (g_FishServer.GetFishConfig().IsTimeItem(pMsg->ItemID))//时间道具
	{
		tm NowTime;
...
bool FishServer::OnHandleAddUserMail(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	if (pMsg->MailInfo.SrcUserID != 0 && msg->Result)//送礼
	{
...
bool FishServer::OnHandleQueryRoleInfoByGameID(BYTE Index, BYTE ClientID, NetCmd* pCmd)
{
	...
	if (m_Sql[Index].Select(SqlStr, 0, pTable1, true) && pTable1.Rows() == 1)
	{
		...
		msg->RoleInfo.dwCashpoint = pTable1.GetUint(0, 15);       //点券
		msg->RoleInfo.byUsingLauncher = pTable1.GetByte(0, 16);;//当前使用的炮台
		msg->RoleInfo.MaxRateIndex = pTable1.GetByte(0, 17);;//倍率
