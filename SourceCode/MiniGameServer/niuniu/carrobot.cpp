...
DWORD CCarRobot::m_dwApplistChangeTime;
BYTE	CCarRobot::m_byAppCountLimit;

DWORD	CCarRobot::m_dwRobotCountChangeTime;
BYTE	CCarRobot::m_byRobotCount;

//////////////////////////////////////////////////////////////////////////
//构造函数
CCarRobot::CCarRobot(DWORD dwRobotid)
{
	//游戏变量
...
//时间消息
bool  CCarRobot::OnEventTimer(UINT nTimerID)
{
	switch (nTimerID)
	{
	case IDI_CHECK_BANKER:		//检查上庄
	{
									...
									if (ApplyListLength()<CCarRobot::m_byAppCountLimit&&!g_FishServer.GetCarManager().HaveApply(UserId()))//BankerId() == 0 ||
									{
										//空庄
										m_nWaitBanker++;
										...
										if (m_bBnkerRobot&& m_nWaitBanker > m_nRobotWaitBanker)
										{
											...
											if (UserGold() > GameConfig().JoinBankerGlobelSum)
											{
												...
												m_nWaitBanker = 0;
												g_FishServer.GetCarManager().OnRoleJoinWriteBankerList(UserId());
											}
										}
									}
									//else if (BankerId() != 0)
	...
	{
									g_FishServer.GetCarManager().OnRoleCanelBanker(UserId());
									return false;
	}
	...
	default:
	{
			   if (nTimerID >= IDI_PLACE_JETTON && nTimerID <= IDI_PLACE_JETTON + MAX_CHIP_TIME)
			   {
				   ...
				   for (int i = 0; i < MAX_CAR_ClientSum; i++)
				   {
					   ...
					   if (i == MAX_CAR_ClientSum - 1)	return false;
				   }
				   ...
				   g_FishServer.GetCarManager().OnRoleBetGlobel(UserId(), nChipArea, nJetton);
...
//游戏空闲
void CCarRobot::EndGame()
{
	if (!m_bAction)
	{
		return;
	}
	m_VecOxTimer.push_back(new CarTimer(IDI_BANK_OPERATE, (WORD)rand() % (GameConfig().EndWriteSec / 2) + 1));
	...
	if (BankerId() == UserId())
	{
		...
		if (m_nBankerCount >= m_nRobotBankerCount)
		{
			...
			m_VecOxTimer.push_back(new CarTimer(IDI_GIVEUP_BANKER, (WORD)rand() % (GameConfig().EndWriteSec / 2) + 1));
			bMeGiveUp = true;
		}
	}
	//检查上庄
	if (BankerId() != UserId() || bMeGiveUp)
	{
		...
		m_VecOxTimer.push_back(new CarTimer(IDI_CHECK_BANKER, (WORD)rand() % (GameConfig().EndWriteSec / 2) + 1));
	}
}
void CCarRobot::AdjustGold(DWORD dwGold)
{
...
void CCarRobot::StartGame()
{
	...
	for (int i = 0; i < m_nChipTime; i++)
	{
		...
		m_VecOxTimer.push_back(new CarTimer(IDI_PLACE_JETTON + i + 1, nElapse));
	}
...
bool CCarRobot::CalcJettonRange(LONGLONG lMaxScore, LONGLONG lChipLmt[], int & nChipTime, int lJetLmt[])
{
	//定义变量
...
bool CCarRobot::Update()
{
	for (vector<CarTimer*>::iterator it = m_VecOxTimer.begin(); it != m_VecOxTimer.end();)
	{
		if (--(*it)->m_nTime == 0)
		{
...
DWORD CCarRobot::UserId()
{
	return m_dwRobotid;
}
DWORD CCarRobot::UserGold()
{
	Role* pRole = g_FishServer.GetRoleManager().QueryRole(m_dwRobotid);
	if (pRole)
	{
...
DWORD CCarRobot::BankerId()
{
	return g_FishServer.GetCarManager().BankerId();
}

DWORD CCarRobot::BankerGold()
{
	return g_FishServer.GetCarManager().BankerGold();
}

WORD  CCarRobot::ApplyListLength()
{
	return g_FishServer.GetCarManager().ApplyListLength();
}


tagCarConfig CCarRobot::GameConfig()
{
	return g_FishServer.GetFishConfig().GetFishMiNiGameConfig().carConfig;
}
/////////////

CCarRobotManager::CCarRobotManager()
{
	srand(GetTickCount());
}
...
//void CCarRobotManager::Init(NiuNiuManager *pgame)
//{
//	m_pGame = pgame;
//}

void CCarRobotManager::Update()
{
	if ((GetTickCount() - CCarRobot::m_dwRobotCountChangeTime) / 1000 > (DWORD)(g_FishServer.GetFishConfig().GetFishMiNiGameConfig().carConfig.RobotCountInterval * 60))//总人数控制
	{
		CCarRobot::m_dwRobotCountChangeTime = GetTickCount();
		SYSTEMTIME st;
		GetLocalTime(&st);
		for (vector<RobotCarCountControl>::iterator it = g_FishServer.GetFishConfig().GetFishMiNiGameConfig().carConfig.VecRobotCount.begin(); it != g_FishServer.GetFishConfig().GetFishMiNiGameConfig().carConfig.VecRobotCount.end(); it++)
		{
			if (st.wHour >= (*it).StartTime&&st.wHour <= (*it).EndTime)
			{
				...
				{
					CCarRobot::m_byRobotCount = byCount1 + rand() % (byCount2 - byCount1 + 1);
				}
				break;
			}
		}
	...
	if ((GetTickCount() - CCarRobot::m_dwApplistChangeTime) / 1000>(DWORD)(g_FishServer.GetFishConfig().GetFishMiNiGameConfig().carConfig.RobotListCountInterval * 60))//申请列表人数限制
	{
		CCarRobot::m_dwApplistChangeTime = GetTickCount();
		BYTE  bymlistmin = g_FishServer.GetFishConfig().GetFishMiNiGameConfig().carConfig.RobotListCountMin;
		BYTE  bylistmax = g_FishServer.GetFishConfig().GetFishMiNiGameConfig().carConfig.RobotListCountMax;
		if (bymlistmin <= bylistmax)
		{
			CCarRobot::m_byAppCountLimit = bymlistmin + rand() % (bylistmax - bymlistmin + 1);
		}
	}

	if (m_allrobot.size() < CCarRobot::m_byRobotCount)
	{
		RobotUserInfo *pInfo = g_FishServer.GetRobotManager().GetNonUseRobot();
		if (pInfo)
		{
			CCarRobot *probot = new CCarRobot(pInfo->dwUserID);//分钟到秒
			g_FishServer.GetCarManager().OnRoleJoinCarTable(pInfo->dwUserID);
			m_allrobot.push_back(probot);
		}
	}

	for (vector<CCarRobot*>::iterator it = m_allrobot.begin(); it != m_allrobot.end();)
	{
		if (!(*it)->Update() && g_FishServer.GetCarManager().IsGameEnd())
		{
			g_FishServer.GetRobotManager().OnNonUseRobot((*it)->UserId());
			g_FishServer.GetCarManager().OnRoleLeaveCarTable((*it)->UserId());
			delete (*it);
...
void CCarRobotManager::StartGame()
{
	for (vector<CCarRobot*>::iterator it = m_allrobot.begin(); it != m_allrobot.end(); it++)
	{
		(*it)->StartGame();
	}
}

void CCarRobotManager::EndGame()
{
	for (vector<CCarRobot*>::iterator it = m_allrobot.begin(); it != m_allrobot.end(); it++)
	{
		(*it)->EndGame();
...
bool CCarRobotManager::IsRobot(DWORD dwUserid)
{
	for (vector<CCarRobot*>::iterator it = m_allrobot.begin(); it != m_allrobot.end(); it++)
	{
...
USHORT CCarRobotManager::RobotCount()
{
