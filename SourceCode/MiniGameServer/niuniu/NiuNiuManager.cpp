...
void NiuNiuManager::OnTableJoinEnd()
{
	...
	for (BYTE i = 0; i < MAX_NIUNIU_ClientSum; ++i)
		TotalAllAddGlobel += m_TableBetAreaGlobel[i];//4个桌子下币数组

	OnceBrandList pBrand;//生成一副新的牌
	//处理结果 
	//1.我们先试图生成 全部的套牌 
	for (BYTE i = 0; i < MAX_NIUNIU_ClientSum + 1; ++i)//5poker
	{
		for (BYTE j = 0; j < MAX_NIUNIU_BrandSum; ++j)
		{
	...
	for (BYTE i = 0; i < MAX_NIUNIU_ClientSum; ++i)
		msg.TableBetAreaGlobel[i] = m_TableBetAreaGlobel[i];//每个桌子下的住
	for (BYTE i = 0; i < MAX_NIUNIU_ClientSum + 1; ++i)
	{
		msg.TableBrandResult[i] = m_TableBrandResult[i];//每个桌子上的牌
		for (BYTE j = 0; j < MAX_NIUNIU_BrandSum; ++j)
			msg.TableBrandArray[i][j] = m_TableBrandArray[i][j];//25poker
	}

	m_NiuNiuResultLog.SetResult(m_TableBrandResult);//根据当局的结果设置数据

	msg.TableGameSum = m_NiuNiuResultLog.LogSum;//游戏局数
	msg.TableResultLog = m_NiuNiuResultLog.ResultLog;//游戏结果
