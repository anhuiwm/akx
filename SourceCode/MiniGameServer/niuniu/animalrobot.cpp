...
{
	if ((GetTickCount() - CAnimalRobot::m_dwRobotCountChangeTime) / 1000 > (DWORD)(g_FishServer.GetFishConfig().GetFishMiNiGameConfig().dialConfig.RobotCountInterval * 60))//总人数控制
	{
	...
	if ((GetTickCount() - CAnimalRobot::m_dwApplistChangeTime) / 1000>(DWORD)(g_FishServer.GetFishConfig().GetFishMiNiGameConfig().dialConfig.RobotListCountInterval * 60))//申请列表人数限制
	{
	...
	if (m_allrobot.size() < CAnimalRobot::m_byRobotCount)
	{
		...
		if (pInfo)
		{	
			CAnimalRobot *probot = new CAnimalRobot(pInfo->dwUserID);//分钟到秒
