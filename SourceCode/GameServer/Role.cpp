...
void CRole::SetRoomLauncher()
{
	...
	//进房间不要重置了m_pRoleEx->GetRoleLauncherManager().ResetUsingLauncher();
...
bool CRole::IsCanAccelerate()//能否使用锁定技能
{
	//return false;//wm todo
	DWORD NowTime = timeGetTime();
...
bool CRole::IsUseAccelerate()//加速技能期间返回true
{
	//return false;//wm todo
	DWORD NowTime = timeGetTime();
...
{
	return 	m_pConfig->BulletConsume(/*m_LauncherType*/GetLauncherType())*m_pConfig->BulletMultiple(m_byMinRate);//最小消耗的子弹
}
