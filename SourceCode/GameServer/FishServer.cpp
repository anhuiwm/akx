...
bool FishServer::InitServer(int Index)
{
	...
	m_GameRobotManager.OnLoadAllGameRobot(pGameConfig->BeginRobotUserID, pGameConfig->EndRobotUserID);//加载新的玩家
	if (!ConnectCenter())
	{
		while (true)
		{
...
{
	//m_RobotManager.OnInit(m_FishConfig.BeginRobotUserID, m_FishConfig.EndRobotUserID);//加载新的玩家
}
...
void FishServer::OnHandleUseRMB(CG_Cmd_UseRMB* pMsg)
{
	...
	else
	{
		...
		if (Iter->second.IsAddCashpoint())
		{
			...
			Res = pRole->ChangeRoleCashpoint(AddMoney, TEXT("实际充值点券"));
		...
		{
			bool Res = pRole->ChangeRoleCashpoint(AddMoney, TEXT("充值一元豪礼点券"));
			if (Res)
			{
				//pRole->OnHandleEvent(false, true, false, ET_Recharge_One, 0, Iter->second.dDisCountPrice);//一元豪礼活动
				RealCharge = true;
			}
		}
		...
		{
			g_DBLogManager.LogUserRechargeLogToDB("游戏服务器充值失败:玩家在线", OrderID, pMsg->rechargeInfo.UserID, ChannelCode, ChannelOrderID, ChannelLabel, pMsg->rechargeInfo.ShopItemID, pMsg->rechargeInfo.Price, pMsg->rechargeInfo.FreePrice, 0, 0, Iter->second.IsAddGlobel() ? Iter->second.AddMoney : 0, Iter->second.IsAddCurrcey() ? Iter->second.AddMoney : 0, Iter->second.IsAddReward() ? Iter->second.RewardID : 0, SendLogDB);
		}
		FreeVec(pVec);
		free(OrderID);
		free(ChannelCode);
...
bool FishServer::OnHandleDataBaseLoadUserMailSendItem(NetCmd* pCmd)//送道具  送和接收都有
{
	//接收到DBO的命令了 我们可以开始处理
		...
		case CL_ChangeRoleRateValue:
			{
				...
				if (RateIndex > GetTableManager()->GetGameConfig()->GetRateQianPaoID())//千炮以上
				{
					return false;
				}
				//DWORD material_num = GetTableManager()->GetGameConfig()->RateMaterial(RateIndex);//锻造材料数量
				//if (material_num > 0)//必须锻造
				//{
				//	return false;
				//}
...
bool FishServer::OnHandleForge(ServerClientData* pClient, NetCmd* pCmd)
{
	...
	switch (pCmd->SubCmdType)
	{
		case CL_FORGE:
		{
			...
			do 
			{
				BYTE RateIndex = pMsg->RateIndex;//玩家需要解锁的倍率
				if (RateIndex != pRole->GetRoleRate().GetCanUseMaxRate() + 1)//倍率档次必须一级一级加
				{
					break;// return true;//无须继续开启了
				}
				...
				//提前判断材料原石够不够
				DWORD material_num = GetTableManager()->GetGameConfig()->RateMaterial(RateIndex);//锻造材料数量
				//if (material_num == 0)//不需要锻造解锁
				//{
				//	break;
				//}

				DWORD rough_num = GetTableManager()->GetGameConfig()->RateRough(RateIndex);//深海原石数量 
				...
				if (!pRole->GetItemManager().OnQueryDelUserItemList(pVec, UseCurreyNum, true))//先查询
				{
					break;// return false;
				}
				...
				pRole->GetItemManager().OnQueryDelUserItemList(pVec, UseCurreyNum, false);//扣除
				if (num > succRate)//不成功
				{
					msg.Result = true;
					//msg.Success = false;
		...
		case CL_QueryUserByGameID:
			{
				...
				//{
					...
					else
					{
						...
						msg.DetailRoleInfo.dwCashpoint = pDestRole->GetRoleInfo().dwCashpoint;       //点券
						msg.DetailRoleInfo.byUsingLauncher = pDestRole->GetRoleInfo().byUsingLauncher;//当前使用的炮台
						msg.DetailRoleInfo.MaxRateIndex = pDestRole->GetRoleRate().GetCanUseMaxRate();//倍率
...
bool FishServer::OnHandleDataBaseLoadQueryUserInfoByGameID(NetCmd* pCmd)
{
    ...
    //if (pMsg->Type == 0)//名片
	{
		...
		msg.DetailRoleInfo.dwCashpoint = pMsg->RoleInfo.dwCashpoint;       //点券
		msg.DetailRoleInfo.byUsingLauncher = pMsg->RoleInfo.byUsingLauncher;//当前使用的炮台
		msg.DetailRoleInfo.MaxRateIndex = pMsg->RoleInfo.MaxRateIndex;//倍率
	...
	case CG_GetGlobelTaskInfo:
		{
			...
			//wm debug m_RoleManager.OnUpdateRoleTask();//关系玩家全部的任务 按中央服务器控制进行
			return true;
		}
	}
	return true;
...
void FishServer::SendBroadCast(CRoleEx* pRole,BYTE Type, const TCHAR* pName, DWORD ItemID, DWORD ItemNum)
{
	...
	//_sntprintf_s(msgInfo, CountArray(msgInfo)-1, TEXT("土豪来了！尊贵的%%s[贵族%%d]登陆了游戏！"));
    ...
    if(Iter != GetFishConfig().GetBroadCastMap().end())
	{
		...
		{
			//_sntprintf_s(msgInfo, CountArray(msgInfo)-1, TEXT("土豪来了！尊贵的%s[贵族%d]登陆了游戏！"), pRole->GetRoleInfo().NickName, pRole->GetRoleInfo().VipLevel);
		...
		//<Broadcasts>
		//	<Broadcast Type = "1" StepNum = "500" StepMin = "50" MessageInfo = "土豪来了！尊贵的[FFFF00]%s[-][贵族[FFFF00]%d[-]]登陆了游戏！" OnceStepNum = "1" OnceStepSec = "3" MessageInfoColor = "4294967295" / >
		//	<Broadcast Type = "2" StepNum = "500" StepMin = "50" MessageInfo = "首充奖励！恭喜[FFFF00]%s[-]，获得首充大礼包！" OnceStepNum = "1" OnceStepSec = "3" MessageInfoColor = "4294967295" / >
		//	<Broadcast Type = "3" StepNum = "500" StepMin = "50" MessageInfo = "众人膜拜！恭喜[FFFF00]%s[-]，成功晋升贵族[FFFF00]%d[-]！" OnceStepNum = "1" OnceStepSec = "3" MessageInfoColor = "4294967295" / >
		//	<Broadcast Type = "4" StepNum = "500" StepMin = "50" MessageInfo = "暴富啦！恭喜[FFFF00]%s[-][贵族[FFFF00]%d[-]]，使用[FFFF00]%d[-]倍率击杀[FFFF00]%s[-]鱼获得[FFFF00]%d[-]金币！" OnceStepNum = "1" OnceStepSec = "3" MessageInfoColor = "4294967295" / >
		//	<Broadcast Type = "5" StepNum = "500" StepMin = "50" MessageInfo = "太棒了！恭喜[FFFF00]%s[-][贵族[FFFF00]%d[-]]，击杀[FFFF00]%s[-]鱼获得[FFFF00]%d[-]个[FFFF00]%s[-]弹头！" OnceStepNum = "1" OnceStepSec = "3" MessageInfoColor = "4294967295" / >
		//	<Broadcast Type = "6" StepNum = "500" StepMin = "50" MessageInfo = "太牛了！恭喜[FFFF00]%s[-][贵族[FFFF00]%d[-]]，在商城中使用[FFFF00]%d[-]红包兑换了[FFFF00]%s[-]道具！" OnceStepNum = "1" OnceStepSec = "3" MessageInfoColor = "4294967295" / >
		//< / Broadcasts>
	...
	case CL_Recharge:
		{
			...
			//else
			{
				...
				if (Iter->second.IsAddCashpoint())
				{
					...
					Res = pRole->ChangeRoleCashpoint(AddMoney, TEXT("测试充值点券"));
					if (Res && pRole->IsFirstPayCashpoint(Index))
					{
						pRole->ChangeRoleIsFirstPayCashpoint(Index);
						RealCharge = true;
					...
					if (Res)
					{
					//	pRole->OnHandleEvent(false, true, false, ET_Recharge_First, 0, Iter->second.dDisCountPrice);//首充活动
						RealCharge = true;
					}
				...
				else if (Iter->second.IsAddCashOne())
				{
					...
					Res = pRole->ChangeRoleCashpoint(AddMoney, TEXT("测试充值一元豪礼点券"));
					if (Res)
					{
						RealCharge = true;
						//pRole->OnHandleEvent(false, true, false, ET_Recharge_One, 0, Iter->second.dDisCountPrice);//一元豪礼活动
				...
				{
					if (pRole->GetCashpoint() < Iter->second.dDisCountPrice)//点券不够
					{
						return false;
					}
					...
					Res = pRole->ChangeRoleCurrency(AddMoney, TEXT("点券兑换钻石"));//充值的金币 立刻保存起来
					if (Res)
					{
						pRole->ChangeRoleCashpoint(Iter->second.dDisCountPrice*-1, TEXT("点券兑换金币"));
					}
				...
				else if (Iter->second.IsAddGlobel())
				{
					...
					Res  = pRole->ChangeRoleGlobe(AddMoney, true, true,true);//充值的金币 立刻保存起来
					if (Res)
					{
						pRole->ChangeRoleCashpoint(Iter->second.dDisCountPrice*-1, TEXT("点券兑换金币"));
					}
...
bool FishServer::OnHandleTCpNetworkOperate(ServerClientData* pClient, NetCmd* pCmd)
{
	...
	case CL_GetPhoneVerificationNum://获取验证码
		{
		    return true;//服务器只做校验了 
...
void FishServer::OnReloadConfig()
{
	...
	{
		ShowInfoToWin("重新加载Config失败");
		return;
	}
	ShowInfoToWin("重新加载Config成功");
}
		...
		case MG_TableJoinEnd:
			{
				...
				if (!pRole)
				{
					...
					else
					{
						...
						MailInfo.bIsRead = false;
						//比赛的内容需要特殊的处理 我们想要一个 特殊的转义字符串 客户端 和 服务器通用的 
						_sntprintf_s(MailInfo.Context, CountArray(MailInfo.Context), TEXT("因为您牛牛游戏失败 扣除您%lld 的金币"), pMsg->AddGlobelSum*-1);
						MailInfo.RewardID = 0;
