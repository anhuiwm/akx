...
public:
	...
	void SetFrozenTime(DWORD addTime)
	{
		DWORD NowTime = timeGetTime();
		m_dwFrozenTime = NowTime + addTime;
	}
	...
	bool IsFrozen()
	{
		DWORD NowTime = timeGetTime();
		//m_dwFrozenTime = NowTime + addTime;
		if (m_dwFrozenTime != 0 && m_dwFrozenTime >= NowTime)
			return true;
		return false;
	}
	...
	UINT LaunchBossFish(USHORT& startID, int fishGroup, bool bforceLaunch, UINT pcakgeType = -1)
	{
		//fishGroup = 158;
		startID = m_FishID;
		GroupData *pGroup = FishResManager::Inst()->GetGroup(fishGroup);

		if (pGroup == NULL || pGroup->FishGroupByPathGroup)
		{
			return -1;
