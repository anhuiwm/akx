...
public:
	...
	virtual void  GetSkillFrozenReduction(byte &speedScaling, byte &duration1, byte& duration2, byte &duration3) = 0;
	//捕获鱼，返回金币数量
	virtual UINT  CatchFish(PlayerID playerID, USHORT fishIndx, CatchType catchType, byte subType, byte packge, byte rate) = 0;
	virtual UINT   CatchFishGold(USHORT fishIndx, CatchType catchType, byte subType, byte byRateIndex) = 0;
	...
	virtual USHORT FishRewardDrop(PlayerID id, BYTE byPackageType,USHORT byType, bool& dropBullet, UINT goldNum = 0) = 0;
	virtual CRoleEx* GetRole(PlayerID id) = 0;
	//是否是闪电鱼，单纯的判断一下。
	...
	virtual BulletType GetBulletNatureType(USHORT launcherType)=0;
	virtual DWORD GetBulletNatureIntervalTime(USHORT launcherType)=0;
	virtual int GetBulletNatureShootcount(USHORT launcherType)=0;
	virtual int GetBulletNatureRebound(USHORT launcherType)=0;
	virtual int GetBulletNatureLock(USHORT launcherType) = 0;
    virtual bool IsBossFish(BYTE id) = 0;
	virtual __int64 GetStaticStockScore(int type) = 0;
	virtual float GetStockTax(int type) = 0;
	virtual void AddStaticStockScore(int type, float score) = 0;
	virtual float StockRate(int type, __int64 stockscore) = 0;
};
