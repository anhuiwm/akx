	...
	case CMD_BULLET:
		{
			...
			//if (player->IsUseAccelerate())//使用加速技能时候
			//{
			...
			else if (player->CheckFire(tabletype, pBullet->LauncherType, mulConsume))
			{
				...
				else
				{
					...
					//InnerFishLog(L"发射子弹, pos:%f, %f, %f, dir:%f, %f, %f, speed:%f", pos.x, pos.y, pos.z, dir.x, dir.y, dir.z, speed);
					//LogInfoToFile("WmBulletLog.txt", "table=%d, seat=%d ,发射子弹:%d, launcherType:%d, byte:%d, dir:%f, %f, %f, pos:%f, %f, %f", player->GetTableID(), player->GetSeatID(), pBullet->BulletID, player->GetLauncherType(), FishCallback::GetFishSetting()->GetBulletSpeed(player->GetLauncherType()), dir.x, dir.x, dir.z, pos.x, pos.y, pos.z);
					bool ret = m_pScene->GetBulletMgr()->Launch(id, pBullet->BulletID, player->GetLauncherType(), player->GetRateIndex(), speed, pBullet->Angle, dir, pos, pBullet->ReboundCount, pBullet->LockFishID, x, y, skillID,VipSkill,tabletype);
					if (!ret)
					{
...
void FishDesk::ProcessSkillFrozen(CRole *player)
{
	...
	ReductionToFloat(speedScl, duration1, duration2, duration3, fSpdScl, fDurationList);
	//int count = RandRange(numMin, numMax);
	//count = min(count, 1000);//最多64条鱼受影响
