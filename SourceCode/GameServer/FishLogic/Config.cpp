...
enum CANNON_TYPE //nature:炮台属性 默认0正常模式,1散弹属性,2定向爆炸穿透链锁,3穿透反弹,4锁定穿透
{
	CANNON_0,  //正常属性                  nature="0"
	CANNON_1,  //正常						 nature="0"
	CANNON_2,  //正常						 nature="0"
	CANNON_3,  //正常						 nature="0"
	CANNON_4,  //散弹 					 nature="1"
	CANNON_5,  //散弹						 nature="1"
	CANNON_6,  //锁定穿透				   nature="4"
	CANNON_7,  //锁定穿透				   nature="4"
	CANNON_8,  //定向爆炸穿透				 nature="2"
	CANNON_9,  //穿透反弹				   nature="3"
	CANNON_10,  //限时 定向爆炸穿透		   nature="2"
	CANNON_11,  //限时 正常				   nature="0"
	CANNON_12,  //限时 锁定穿透			 nature="4"
	CANNON_13,  //限时 散弹				   nature="1"
};
...
float CConfig::CatchChance(PlayerID player, USHORT bulletId, byte bulletType, byte fishType, int byOrder, int byMaxCatch, BYTE byPackageType,bool bLocked)
{
	...
	//添加机器人功能是 触发更加频繁 暂时关闭assert 2017.5.17
...
USHORT CConfig::FishRewardDrop(PlayerID id, BYTE byPackageType,USHORT byType, bool& dropBullet, UINT goldNum)
{
	...
	if (pPlayer)
	{
		...
		if (pRoleEx&&!pRoleEx->GetRoleMonth().IsInMonthTable())
		{
			...
			bool bBossFish = IsBossFish(byType);// m_Fish[byType].isfishboss == 1);//过滤boss鱼
			if (!bBossFish)
			{
				float fdropother = 1;
