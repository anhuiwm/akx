...
void GameTable::SendTableRoleInfoToClient(DWORD dwUserID)
{
	...
	for (BYTE i = 0; i < m_RoleManager.GetMaxPlayerSum(); ++i)
	{
		...
		if (pOtherUser->GetID() != pUser->GetID())
		{
			...
			msg.UserInfo.dwCashpoint = pOtherUser->GetRoleExInfo()->GetRoleInfo().dwCashpoint;       //点券
			msg.UserInfo.byUsingLauncher = pOtherUser->GetRoleExInfo()->GetRoleInfo().byUsingLauncher;//当前使用的炮台
			msg.UserInfo.MaxRateIndex = pOtherUser->GetRoleExInfo()->GetRoleRate().GetCanUseMaxRate();//倍率
...
void GameTable::SendRoleJoinInfo(DWORD dwUserID)
{
	...
	msgUser.UserInfo.dwCashpoint = pUser->GetRoleExInfo()->GetRoleInfo().dwCashpoint;       //点券
	msgUser.UserInfo.byUsingLauncher = pUser->GetRoleExInfo()->GetRoleInfo().byUsingLauncher;//当前使用的炮台
	msgUser.UserInfo.MaxRateIndex = pUser->GetRoleExInfo()->GetRoleRate().GetCanUseMaxRate();//倍率
