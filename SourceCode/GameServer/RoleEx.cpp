...
void CRoleEx::OnUserLoadFinish(bool IsLogonGameServer)//当当前对象加载完毕的时候
{
	...
	{
		SendLoginReward();
		ChangeRoleLastOnlineTime();//会改最后在线时间
	}
	SetAfkStates(false);
...
bool CRoleEx::ChangeRoleGuideStep(BYTE byStep)
{
	...
	{
		OnAddRoleRewardByRewardID(g_FishServer.GetFishConfig().GetSystemConfig().NewFishRewardID, TEXT("领取新手奖励"));
	}
...
bool CRoleEx::ChangeSaveRoleGlobe(__int64 saveGlobe)
{
	...
	if (saveGlobe < 0 || saveGlobe >= g_FishServer.GetFishConfig().GetSystemConfig().MaxGobelSum)//金币到达上限了 无法添加金币
		return false;
...
bool CRoleEx::ChangeRoleGlobe(int AddGlobe, bool IsSendToClient, bool IsSaveToDB,bool IsSendToMiniGame,bool IsCatchFish)
{
	...
	if (IsCatchFish)
	{
		ChangeRoleWeekClobeNum(AddGlobe, true, false); // 打鱼得到
	}
	//else
	//{
	//	ChangeRoleWeekClobeNum(0, IsSendToClient, IsSaveToDB); // 打鱼得到
	//}
	return true;
}
...
{
	{
		...
		tagRoleMail	MailInfo;
		MailInfo.bIsRead = false;
		_sntprintf_s(MailInfo.Context, CountArray(MailInfo.Context), TEXT("恭喜您,获得贵族%u每日登录礼包!"), VipLevel);
...
bool CRoleEx::SaveRoleDayOnlineSec(bool clear)
{
   ...
   else
   {
	...
	msgDB.dwUserID = m_RoleInfo.dwUserID;
	msgDB.OnLineSec = m_RoleInfo.OnlineSec;//保存玩家的在线
	g_FishServer.SendNetCmdToSaveDB(&msgDB);
...
bool CRoleEx::ChangeRoleNobilityPoint(DWORD AddSum)
{
	if (AddSum == 0)
		return true;
	m_RoleInfo.NobilityPoint += AddSum;//玩家总充值的数量 单位元 不是分
...
bool CRoleEx::ChangeRoleCheck(bool bCheck)//今日签到
{
	if (bCheck)
	{
...
bool CRoleEx::ChangeRoleDayTaskActiviness(BYTE AddSum, bool clear)//加多少
{
	if (clear)
	{
	...
	else
	{
		...
		{
			m_RoleInfo.DayTaskActiviness += AddSum;//玩家总充值的数量 单位元 不是分
		}
...
bool CRoleEx::ChangeRoleWeekTaskActiviness(BYTE AddSum, bool clear)
{
	...
	else
	{
		...
		{
			m_RoleInfo.WeekTaskActiviness += AddSum;//玩家总充值的数量 单位元 不是分
		}
...
{
	OnHandleEvent(false, true, false, ET_Recharge_First, 0, 1);//首充活动
	g_FishServer.SendBroadCast(this, NoticeType::NT_First_Charge);
	//GetRoleActionManager().OnFinishAction(101, 1);//首冲自动领取
}
...
{
	if (m_IsRobot)//机器人暂时设置false  cpu消耗
	{
		return false;
	}
...
void CRoleEx::UpdateOnlineStatesByMin(bool IsHourChange)
{
	...
	for (;Iter != g_FishServer.GetFishConfig().GetOnlineRewardConfig().m_OnlineRewardMap.end(); ++Iter)
	{
		...
		if (!(TempOnlineRewardStates & (1 << (Iter->first - 1 + 16))))
		{
				...
				)
			{
				...
				//ChangeRoleOnlineRewardStates(TempOnlineRewardStates | (1 << (Iter->first - 1 + 16)));//设置完成  并且推送给客户端数据库
				//GetRoleMessageStates().OnChangeRoleMessageStates(RMT_Online);
			}
		}
	...
	{
		ChangeRoleOnlineRewardStates(TempOnlineRewardStates);//设置完成  并且推送给客户端数据库
		//if (bRMT)
		//{
		//	GetRoleMessageStates().OnChangeRoleMessageStates(RMT_Online);
		//}
...
bool CRoleEx::GetOnlineRewardMessageStates()
{
	...
	for (; Iter != g_FishServer.GetFishConfig().GetOnlineRewardConfig().m_OnlineRewardMap.end(); ++Iter)
	{
		...
		{
			if (!(GetRoleInfo().OnlineRewardStates & (1 << (Iter->first - 1))))//如果未领奖
			{
				return true;
			}
		}
...
bool CRoleEx::SaveAllRoleInfo(bool IsExit)
{
	...
	msg->OnlineSec = GetRoleOnlineSec();//保存玩家的在线秒数
...
{ 
	ChangeRoleOnlineTimeByDay(!States);//控制在线时长用
