...
void ShopManager::OnShellShopItem(CRoleEx* pRole, BYTE ShopID, BYTE ItemIndex, DWORD ItemSum)
{
	...
	if (IterItem->second.ShopType == SIT_Sell)//回收
	{
		if (pRole->GetItemManager().QueryItemCount(IterItem->second.ItemInfo.ItemID) >= ItemSum)
		{
			if (pRole->GetItemManager().OnDelUserItem(IterItem->second.ItemInfo.ItemID, ItemSum))
			{
				...
				pRole->ChangeRoleMedal(IterItem->second.PriceMabel*ItemSum, TEXT("商店回收物品"));
				pRole->ChangeRoleCurrency(IterItem->second.PriceCurrey*ItemSum, TEXT("商店回收物品"));
				msg.Result = true;
			}
		}
	}
	else
	{
		...
		else
		{
			...
			{
				//购买道具类 包括金币
				DBR_Cmd_AddRoleShopNormalItem msgDB;//发送到数据库去
