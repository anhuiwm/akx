...
bool RoleCheck::IsCanCheckNowDay()
{
	...
	////DWORD  WeekCheck = 16;// GetWeek();  第16位表示今天是否签到
	//return ((CheckData& (1 << CheckSign)) == 0);
}


DWORD RoleCheck::GetWeekCheckNum()//下排签到累计次数
{
...
bool RoleCheck::RoleChecking()
{
	...
	////今天可以签到 我们进行设置
	//DWORD CheckData = m_Role->GetRoleInfo().CheckData == 7 ? 1 : (m_Role->GetRoleInfo().CheckData + 1);//周签到加一 第七天循环

	//CheckData = m_Role->GetRoleInfo().CheckData  | (1 << CheckSign);//已经签到
	...
	{
		m_Role->OnAddRoleRewardByRewardID(Iter->second, TEXT("领取签到连续天数奖励"));
		msgCheck.MonthRewardID = Iter->second;
	}
	m_Role->SendDataToClient(&msgCheck);

	//m_Role->ChangeRoleNobilityPoint(10);//添加玩家贵族点数
