...
bool RoleMailManager::OnGetUserMailItem(DWORD MailID)//邮件领取奖励
{
	if (!m_pUser)
	{
		ASSERT(false);
...
bool RoleMailManager::OnAddUserMail(tagRoleMail* pMail, DWORD	dwDestUserID)
{
	...
	else//好友邮件
	{
		...
		else
		{
			...
			DBR_Cmd_AddUserMail msg;
			SetMsgInfo(msg, DBR_AddUserMail, sizeof(DBR_Cmd_AddUserMail));
			msg.dwDestUserID = dwDestUserID;//m_pUser->GetUserID();//当前玩家
			msg.MailInfo = *pMail;
			g_FishServer.SendNetCmdToDB(&msg);
		}
	}
...
bool RoleMailManager::OnBeAddUserMail(tagRoleMail* pMail)
{
	...
	//{
		...
		m_pUser->GetRoleMessageStates().OnChangeRoleMessageStates(RMT_Mail);
	//}
	//else//好友邮件也会在center server 保存
	//{
	//	DBR_Cmd_AddUserMail msg;
	//	SetMsgInfo(msg, DBR_AddUserMail, sizeof(DBR_Cmd_AddUserMail));
	//	msg.dwDestUserID = m_pUser->GetUserID();//m_pUser->GetUserID();//当前玩家
	//	msg.MailInfo = *pMail;
	//	g_FishServer.SendNetCmdToDB(&msg);
	//}
	return true;
...
//	{
//		//玩家在线 我们进行处理
//		pDestRole->GetMailManager().OnAddUserMail(&pMail);//给玩家添加邮件
//	}
//	else
//	{
//		//发送到中央服务器去
