...
void print_json(Json::Value& data)
{
	...
	{
		//cout << *iter << "\t: ";
		LogInfoToFile("Wmjson.txt", "%s", (*iter).c_str());
		...
		{
			LogInfoToFile("Wmjson.txt", "%s", (data[*iter].asString()).c_str());
			//cout << data[*iter].asString() << endl;
		}
		else if (data[*iter].type() == Json::realValue)
		{
			LogInfoToFile("Wmjson.txt", "%f", data[*iter].asDouble());

			//cout << data[*iter].asDouble() << endl;
		}
		else if (data[*iter].type() == Json::uintValue)
		{
			LogInfoToFile("Wmjson.txt", "%u", (data[*iter].asUInt()));

			//cout << data[*iter].asUInt() << endl;
		}
		else
		{
			LogInfoToFile("Wmjson.txt", "%d", (data[*iter].asInt()));
...
bool FishServer::ConnectClient()
{
	...
	pCLientData.AcceptRecvData = true;// false;	//表示接收初始数据
...
uint FishServer::CanConnected(BYTE SeverID, uint ip, short port, void *pData, uint recvSize, char* resData)
{
	...
	if (IsNeed)
	{
		...
		//{
			// 解析http请求头信息  
			int ret = CONNECT_CHECK_FAILED;
			std::istringstream stream((char*)pData);
			std::string reqType;
			...
			//websocketKey = "5utD8WLozSWfLemEbp4ddA==";
			...
			SHA1((const unsigned char*)serverKey.c_str(), serverKey.size(), md); //补位 
			SHA1_Init(&c);  //初始化
			SHA1_Update(&c, serverKey.c_str(), serverKey.size()); //进行hash 
			...
			SHA_CTX c;
			unsigned char md[SHA_DIGEST_LENGTH] = { 0 };
			int ret1 = SHA1_Init(&c);
			// 初始化 SHA Contex, 成功返回1,失败返回0
			SHA1_Update(&c, serverKey.c_str(), serverKey.size());
			...
			printf("\n");
			serverKey = base64Encode((const char*)md, 20, false);
			serverKey += "\r\n";
			strcat(resData, serverKey.c_str());
			strcat(resData, "Upgrade: websocket\r\n\r\n");
			return CONNECT_WEB_SOCKET;// CONNECT_CHECK_OK;
		//}
#endif
	}
	else
...
//wm	if (pCmd->CmdType != Main_Logon)
	...
	if (1)
	{
		CL_Cmd_Json* pMsg = (CL_Cmd_Json*)pCmd;
		if (!pMsg)
		{
			ASSERT(false);
			return;
		...
		LogInfoToFile("Wmjson.txt", "%s", pMsg->data);

		std::string strJosnInfo((char*)pMsg->data, pMsg->GetDataSize());
	...
	//if (m_FishConfig.LogonServerCheck() && !m_FishConfig.CheckServerState())
	//{
	//	if (!m_FishConfig.IsLogonGm(0, pClient->IP))
	//	{
	//		LogInfoToFile("WmGmLogon.txt", "longon::IP=%d",  pClient->IP);
	//		return;
	//	}
	//	LogInfoToFile("WmGmLogon.txt", "longon:: gm IP=%d", pClient->IP);
	//}
	...
	case CL_WeiXinLogon:
		{
			...
			print_json(jsonRoot);
#if 0
			if (jsonRoot["openid"].isNull() ||
				jsonRoot["nickname"].isNull() ||
				jsonRoot["sex"].isNull() ||
			...
			std::string OpenID = jsonRoot["openid"].asString();
			...
			//	LogInfoToFile("WmLogonError.txt", TEXT("客户端版本错误 微信登陆失败"));
			//	return;
			//}
	...
	case OL_ChannelLogon:
		{
			...
			if (pMsg->Result)
			{
				...
				if (len < (64-1)*2)
				{
					memcpy(Channel, pVec[0]->Array , pVec[0]->Length);
					//_sntprintf(Channel, pVec[0]->Length / sizeof(TCHAR), TEXT("%s"), pVec[0]->Array);
					//_stprintf_s(Channel, _tcslen(pVec[0]->Array), TEXT("%s"), pVec[0]->Array);
					std::map<wstring, wstring>::iterator itr = m_FishConfig.GetChannelMap().find(Channel);
					if (itr != m_FishConfig.GetChannelMap().end())
					{
						_stprintf_s(Channel, 64-1, TEXT("%s"), itr->second.c_str());
					}
					else
					{
						_stprintf_s(Channel, 64-1, TEXT("%s"), TEXT("yj"));
					}
				}
				else
				{
				...
				//1.账号名称
				DWORD uidCrc32 = GetCrc32(pVec[2]->Array, pVec[2]->Length/sizeof(TCHAR));
				DWORD channelLabelCrc32 = GetCrc32(pVec[0]->Array, pVec[0]->Length / sizeof(TCHAR));//不可能变化的
				...
				_stprintf_s(msg.AccountName, CountArray(msg.AccountName), TEXT("%s%u%u"),Channel, uidCrc32,channelLabelCrc32);
...
void FishServer::OnUpdateWriteGameServerUserMap(DWORD dwTimer)
{
	...
	{
		//10s
		if (dwTimer - Iter->second.LogTime >= 10000*100)
		{
