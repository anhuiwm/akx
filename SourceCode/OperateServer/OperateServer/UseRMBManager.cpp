...
bool UseRMBManager::OnHandleHttpInfoByUserRecharge(string app, string cbi, string ct, string fee, string pt, string sdk, string ssid, string st, string tcd, string uid, string ver, string sign)
{
	//命令应该可以做到玩家所在的Center所在
	//CustParam 应该包含  UserID  /ShopID 
	DWORD dPrice = strtoul(fee.c_str(), null, 10);
	DWORD FreePrice = 0;
	string BackInfo = Base64Decode(cbi.c_str(), cbi.length());
	...
	LogInfoToFile("WmRecharge.txt", "回调:\
		UserID=%u,ShopItemID=%u,app=%s,cbi=%s,ct=%s,fee=%s,pt=%s,sdk=%s,ssid=%s,st=%s,tcd=%s,uid=%s,ver=%s,sign=%s",
		UserID, ShopItemID, app.c_str(), cbi.c_str(), ct.c_str(), fee.c_str(), pt.c_str(), sdk.c_str(), ssid.c_str(), st.c_str(), tcd.c_str(), uid.c_str(), ver.c_str(), sign.c_str());


	string ChannelCode = sdk;
	string channelOrderid = ssid;
	string channelLabel = sdk;
	string orderid = tcd;
	...
	{
		g_DBLogManager.LogUserRechargeLogToDB("operator server fail:no shopitemid", orderid, UserID, ChannelCode, channelOrderid, channelLabel, ShopItemID, dPrice, FreePrice, 0, 0, 0, 0, 0, SendLogDB);
		//g_DBLogManager.LogUserRechargeLogToDB("1运营服务器验证失败:物品不存在", orderid, UserID, ChannelCode, channelOrderid, channelLabel, ShopItemID, dPrice, FreePrice, 0, 0, 0, 0, 0, SendLogDB);
		return false;
	}
	if (Iter->second.dDisCountPrice * 100 != dPrice)//价格不正确
	{
		g_DBLogManager.LogUserRechargeLogToDB("operator server fail:price erro", orderid, UserID, ChannelCode, channelOrderid, channelLabel, ShopItemID, dPrice, FreePrice, 0, 0, 0, 0, 0, SendLogDB);
		//g_DBLogManager.LogUserRechargeLogToDB("2运营服务器验证失败:价格不正确", orderid, UserID, ChannelCode, channelOrderid, channelLabel, ShopItemID, dPrice, FreePrice, 0, 0, 0, 0, 0, SendLogDB);
		return false;
	}
	...
	if (!pConfig)
	{
		g_DBLogManager.LogUserRechargeLogToDB("operator server fail: config error", orderid, UserID, ChannelCode, channelOrderid, channelLabel, ShopItemID, dPrice, FreePrice, 0, 0, 0, 0, 0, SendLogDB);
		//g_DBLogManager.LogUserRechargeLogToDB("3运营服务器验证失败:系统错误", orderid, UserID, ChannelCode, channelOrderid, channelLabel, ShopItemID, dPrice, FreePrice, 0, 0, 0, 0, 0, SendLogDB);
		return false;
	}
	...
	{
		g_DBLogManager.LogUserRechargeLogToDB("operator server fail: MD5 error", orderid, UserID, ChannelCode, channelOrderid, channelLabel, ShopItemID, dPrice, FreePrice, 0, 0, 0, 0, 0, SendLogDB);
		//g_DBLogManager.LogUserRechargeLogToDB("4运营服务器验证失败:MD5验证失败", orderid, UserID, ChannelCode, channelOrderid, channelLabel, ShopItemID, dPrice, FreePrice, 0, 0, 0, 0, 0, SendLogDB);
		return false;
	}
...
//	if (Iter == g_FishServer.GetFishConfig().GetFishRechargesConfig().m_FishRechargeMap.end())//物品不存在
//	{
//		g_DBLogManager.LogUserRechargeLogToDB("6运营服务器验证失败:物品不存在", orderid, UserID, ChannelCode, channelOrderid, channelLabel, ShopItemID, dPrice, 0, 0, 0, 0, 0, 0, SendLogDB);
//		return false;
//	}
//	if (Iter->second.dDisCountPrice * 100 != dPrice)//价格不正确
//	{
//		g_DBLogManager.LogUserRechargeLogToDB("7运营服务器验证失败:价格不正确", orderid, UserID, ChannelCode, channelOrderid, channelLabel, ShopItemID, dPrice, 0, 0, 0, 0, 0, 0, SendLogDB);
//		return false;
//	}
...
//	if (!pConfig)
//	{
//		g_DBLogManager.LogUserRechargeLogToDB("8运营服务器验证失败:系统错误", orderid, UserID, ChannelCode, channelOrderid, channelLabel, ShopItemID, dPrice, 0, 0, 0, 0, 0, 0, SendLogDB);
//		return false;
//	}
...
//	if (Md5Str.compare(sign) != 0)//验证签名是否正确
//	{
//		g_DBLogManager.LogUserRechargeLogToDB("9运营服务器验证失败:MD5验证失败", orderid, UserID, ChannelCode, channelOrderid, channelLabel, ShopItemID, dPrice, 0, 0, 0, 0, 0, 0, SendLogDB);
//		return false;
//	}
//
