...
bool FishServer::ConnectHttp()
{
	...
	//支付易接回调
	pHttpInitData.AddWebAddr(HT_Recharge, "pay_callback_lj.clkj?", "SUCCESS", true, "app,cbi,ct,fee,pt,sdk,ssid,st,tcd,uid,ver,sign");//,sign2,version,freePrice,sdkCode 可选参数
...
bool FishServer::ConnectHttpClient()
{
	...
	{
		//启动HTTP 客户端
		//1.登陆 HCT_Logon wm登录易接
		//m_ChannelLogonID = m_HttpClient.AddHost("gameproxy.xinmei365.com");
		m_ChannelLogonID = m_HttpClient.AddHost("sync.1sdk.cn");
		
		//3.短信 wm mob
		m_PhoneSMSID = m_HttpClient.AddHost("webapi.sms.mob.com");
		//wm //4.话费 HCT_PhonePay
		//wm m_PhonePayID = m_HttpClient.AddIP(pOperateConfig->PhonePayIP, pOperateConfig->PhonePayPort);//IP Port 配置文件配置
		//wm //5.普通充值
		//wm m_NormalRechargeID = m_HttpClient.AddHost("ipay.iapppay.com", 9999);
		//wm //6.IOS充值
		//wm //m_IOSPayID = m_HttpClient.AddHost("buy.itunes.apple.com");
		//wm //m_IOSPayTestID = m_HttpClient.AddHost("sandbox.iTunes.Apple.com");
		//wm m_IOSPayID = m_HttpClient.AddIP("127.0.0.1", 45435);
		//wm //7.
		//wm m_WeiXinTokenID = m_HttpClient.AddIP("127.0.0.1", 45435);//https api.weixin.qq.com
		//wm //8.
		//wm m_QQTokenID = m_HttpClient.AddIP("127.0.0.1", 45435);//https  graph.qq.com
		//wm 
		//wm 
		//wm m_WeiXinUserInfoID = m_HttpClient.AddIP("127.0.0.1", 45435);
		//wm m_WeiXinFaceImgID = m_HttpClient.AddIP("127.0.0.1", 45435);
		//wm //m_WeiXinUserTokenID = m_HttpClient.AddIP("127.0.0.1", 45435);
		//wm 
		//wm m_QQUserInfoID = m_HttpClient.AddIP("127.0.0.1", 45435);
		//wm m_QQFaceImgID = m_HttpClient.AddIP("127.0.0.1", 45435);
	...
	case HT_Recharge:
		{
		   //if (1)//yijie
		   {
			   bool Result = m_UseRMBManager.OnHandleHttpInfoByUserRecharge(
				   pHttpData->KeyValues[m_UseRMBManager.GetRechargeCrcInfo().app], pHttpData->KeyValues[m_UseRMBManager.GetRechargeCrcInfo().cbi],
				   pHttpData->KeyValues[m_UseRMBManager.GetRechargeCrcInfo().ct], pHttpData->KeyValues[m_UseRMBManager.GetRechargeCrcInfo().fee],
				   pHttpData->KeyValues[m_UseRMBManager.GetRechargeCrcInfo().pt], pHttpData->KeyValues[m_UseRMBManager.GetRechargeCrcInfo().sdk],
				   pHttpData->KeyValues[m_UseRMBManager.GetRechargeCrcInfo().ssid], pHttpData->KeyValues[m_UseRMBManager.GetRechargeCrcInfo().st],
				   pHttpData->KeyValues[m_UseRMBManager.GetRechargeCrcInfo().tcd], pHttpData->KeyValues[m_UseRMBManager.GetRechargeCrcInfo().uid],
				   pHttpData->KeyValues[m_UseRMBManager.GetRechargeCrcInfo().ver], pHttpData->KeyValues[m_UseRMBManager.GetRechargeCrcInfo().sign]
			   );
			   return Result;
		   }
		   //old 
