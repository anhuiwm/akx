...
UINT AECrc32(const void *pData, UINT size, UINT CrcValue)
{
	...
	UINT offsetBytes = (UINT)AEAlignSize(pByteData, 4);
	if (size > offsetBytes)
	{
		size -= offsetBytes;
		for (; offsetBytes; --offsetBytes)
			CrcValue = (CrcValue >> 8) ^ CRC_TABLE[0][(CrcValue & 0xFF) ^ *pByteData++];

		const UINT *pData4 = (const UINT*)pByteData;
		for (UINT i = size >> 3; i; --i)
		{
			UINT v1 = *pData4++ ^ CrcValue;
			UINT v2 = *pData4++;
			CrcValue =
				CRC_TABLE[7][v1 & 0xFF] ^
				CRC_TABLE[6][(v1 >> 8) & 0xFF] ^
				CRC_TABLE[5][(v1 >> 16) & 0xFF] ^
				CRC_TABLE[4][v1 >> 24] ^
				CRC_TABLE[3][v2 & 0xFF] ^
				CRC_TABLE[2][(v2 >> 8) & 0xFF] ^
				CRC_TABLE[1][(v2 >> 16) & 0xFF] ^
				CRC_TABLE[0][v2 >> 24];
		}
		pByteData = (const BYTE*)pData4;
		size &= 7;
	}
	for (; size; --size)
		CrcValue = (CrcValue >> 8) ^ CRC_TABLE[0][(CrcValue & 0xFF) ^ *pByteData++];
...
BOOL AECrc32(AE_CRC_PAIRS &retCrc, const void *pData, UINT size, UINT CrcValue1, UINT CrcValue2)
{
	...
	{
		size -= offsetBytes;
		for (; offsetBytes; --offsetBytes, ++pByteData)
		{
			CrcValue1 = (CrcValue1 >> 8) ^ CRC_TABLE[0][(CrcValue1 & 0xFF) ^ *pByteData];
			CrcValue2 = (CrcValue2 >> 8) ^ CRC_TABLE[0][(CrcValue2 & 0xFF) ^ *pByteData];
		}

		const UINT *pData4 = (const UINT*)pByteData;
		for (UINT i = size >> 3; i; --i)
		{
			UINT v1 = *pData4 ^ CrcValue1;
			UINT v3 = *pData4 ^ CrcValue2;
			++pData4;
			UINT v2 = *pData4++;
			CrcValue1 =
				CRC_TABLE[7][v1 & 0xFF] ^
			...
			CrcValue2 =
				CRC_TABLE[7][v3 & 0xFF] ^
				CRC_TABLE[6][(v3 >> 8) & 0xFF] ^
				CRC_TABLE[5][(v3 >> 16) & 0xFF] ^
				CRC_TABLE[4][v3 >> 24] ^
				CRC_TABLE[3][v2 & 0xFF] ^
				CRC_TABLE[2][(v2 >> 8) & 0xFF] ^
				CRC_TABLE[1][(v2 >> 16) & 0xFF] ^
	...
	for (; size; --size, ++pByteData)
	{
		CrcValue1 = (CrcValue1 >> 8) ^ CRC_TABLE[0][(CrcValue1 & 0xFF) ^ *pByteData];
		CrcValue2 = (CrcValue2 >> 8) ^ CRC_TABLE[0][(CrcValue2 & 0xFF) ^ *pByteData];
	}
	retCrc.Crc1 = ~CrcValue1;
	retCrc.Crc2 = ~CrcValue2;
	return TRUE;
}
...
BOOL	AECrc32(AE_CRC_THREE &retCrc, const void *pData, UINT size, UINT CrcValue1, UINT CrcValue2, UINT CrcValue3)
{
	...
	if (size > offsetBytes)
	{
		...
		for (; offsetBytes; --offsetBytes, ++pByteData)
		{
			CrcValue1 = (CrcValue1 >> 8) ^ CRC_TABLE[0][(CrcValue1 & 0xFF) ^ *pByteData];
			CrcValue2 = (CrcValue2 >> 8) ^ CRC_TABLE[0][(CrcValue2 & 0xFF) ^ *pByteData];
			CrcValue3 = (CrcValue3 >> 8) ^ CRC_TABLE[0][(CrcValue3 & 0xFF) ^ *pByteData];
		}
		...
		for (UINT i = size >> 3; i; --i)
		{
			...
			CrcValue3 =
				CRC_TABLE[7][v4 & 0xFF] ^
				CRC_TABLE[6][(v4 >> 8) & 0xFF] ^
				CRC_TABLE[5][(v4 >> 16) & 0xFF] ^
				CRC_TABLE[4][v4 >> 24] ^
				CRC_TABLE[3][v2 & 0xFF] ^
				CRC_TABLE[2][(v2 >> 8) & 0xFF] ^
				CRC_TABLE[1][(v2 >> 16) & 0xFF] ^
