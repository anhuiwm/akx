...
inline bool CheckChangeInt64Value(_int64 nSrcValue, _int64 ChangeValue)
{
	...
	{
		__int64 DestValue = nSrcValue + ChangeValue;
		if (DestValue > 100000000000)
			return false;
		return true;
	}
	else
	{
		return (ChangeValue*-1) <= nSrcValue;
	}
}
...
inline void SqlitMsg(T* OldMsg, DWORD OldMsgSize, DWORD ArraySum, bool IsSendToClient, std::vector<T*>& pVec)
{
	...
	else
	{
		...
		DWORD SqlitSum = ((ArraySum%MsgMaxArraySum == 0) ? (ArraySum / MsgMaxArraySum) : (ArraySum / MsgMaxArraySum + 1));
