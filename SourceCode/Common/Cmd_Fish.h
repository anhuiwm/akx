...
#define MAX_TABLE_TYPE 4  //最大房间索引
...
struct int256Handle
{
	...
	//static void SetBitStatesTrue(int256& p, BYTE BitIndex)
	//{
	//	BYTE ArrayIndex = static_cast<BYTE>(BitIndex / 8);
	//	BYTE ChangeIndex = static_cast<BYTE>(BitIndex % 8);
	//	BYTE BitValue = static_cast<BYTE>(1 << ChangeIndex);
	...
	//static void SetBitStatesFalse(int256& p, BYTE BitIndex)
	//{
	//	BYTE ArrayIndex = static_cast<BYTE>(BitIndex / 8);
	//	BYTE ChangeIndex = static_cast<BYTE>(BitIndex % 8);
	//	BYTE BitValue = static_cast<BYTE>(1 << ChangeIndex);
	...
	//static void ClearBitStatesBeforeIndex(int256& p, BYTE BitIndex)
	//{
	//	for (BYTE i = 1; i <= BitIndex; ++i)//玩家的默认倍率 只可以是前32个
	//	{
	//		int256Handle::SetBitStates(p, i, false);
	//	}
	//}

	//static void ClearBitStatesAfterIndex(int256& p, BYTE BitIndex)
	//{
	//	for (BYTE i = BitIndex+1; i < 255; ++i)//玩家的默认倍率 只可以是前32个       //必须用<=否则会有死循环
	//	{
	//		int256Handle::SetBitStates(p, i, false);
	//	}
	//}
...
struct tagRoleInfo
{
	...
	//每天在线的秒数
	DWORD			OnlineSec;
	//DWORD			OnlineCompleteStates;//玩家当前完成在线奖励的记录 最大 32个 
	...
	BYTE			FirstPayCashpointStates;//是否首冲
	//新的
	...
	DWORD           dwCashpoint;       //点券
	DWORD           dwGoldBulletNum;   //黄金弹头数量
	BYTE            byUsingLauncher;   //当前使用的炮台
	DWORD           NobilityPoint;     //贵族点
	BYTE            AddupCheckNum;     //15天 累计签到
	BYTE            DayTaskActiviness; //日任务活跃度
	BYTE            WeekTaskActiviness;//周任务活跃度
	BYTE            GuideStep;         //引导步骤
	__int64         WeekGlobeNum;      //周金币
	bool            IsCheckToday;      //今日是否签到
	DWORD           SendGoldBulletNum  ;//每日赠送黄金弹头数量
	DWORD           SendSilverBulletNum;// 每日赠送黄金弹头数量
	DWORD           SendBronzeBulletNum;// 赠送黄金弹头数量
};

//
////点击查询其他一个玩家信息
//struct tagQueryOneRoleInfo
//{
//	DWORD			dwUserID;
//	TCHAR			NickName[MAX_NICKNAME + 1];
//	WORD			wLevel;//玩家点击
...
struct tagRoleAddressInfo
{
	...
	TCHAR           EmailCode[7];
	TCHAR           EntityItemStates[MAX_NAME_LENTH+1];//省
	TCHAR           EntityItemCity[MAX_NAME_LENTH+1];//市
	bool            isMale;//true 男
};
...
enum EItemType
{
	...
	IT_Horn		  =18,//喇叭
	IT_Cashpoint = 19,//点券
	IT_NobilityPoint = 21,//贵族点

};
struct tagComposeConfig
{
	DWORD	ItemID;//物品ID
	DWORD   ItemNum;
	DWORD   ComposeID;
...
struct tagSendItemTypeConfig
{
	BYTE    TypeID;//物品类型
	BYTE    CanType;//=0表示不能赠送，=1表示可以送 =2可以背包使用  =3可送也可使用 =4可送可回收
};
...
struct tagRoleExchangeInfo
{
	DWORD		ItemID = 0;//物品ID
	DWORD		ItemSum = 0;//物品数量
	__int64     ShopTime = 0;//物品结束时间
	DWORD       MedalNum = 0;//奖牌数量
	//BYTE        ShopType = 0;//0普通  1实体物品
	bool        bCheck = false;//审核过
};
...
struct tagRoleMail   //从数据库读取出来的邮件信息
{
	...
	WORD		RewardID;//itemID
	DWORD		RewardSum;//奖励的数量
	bool		bIsRead;//是否已经阅读
	bool		bIsExistsReward;//是否存在奖励
};

struct tagRoleMailRecord   //从数据库读取出来的邮件记录信息
{
	DWORD		RecordID;
	bool        bSend; // true 为发送  false 为接收
	DWORD		SrcUserID;//发送方的ID 或者收取方ID
	TCHAR		SrcNickName[MAX_NICKNAME + 1];
	__int64		SendTimeLog;//发送时间
	WORD		RewardID;//邮件物品
	DWORD		RewardSum;//物品数量
};

struct tagRoleSendItem  
{
	DWORD		SrcUserID;//发送方的ID 或者收取方ID
	TCHAR		SrcNickName[MAX_NICKNAME + 1];
	DWORD		DestUserID;
	TCHAR		DestNickName[MAX_NICKNAME + 1];
	__int64		SendTime;//发送时间
	WORD		RewardID;//邮件物品
	DWORD		RewardSum;//物品数量
	bool        bIsRead;;//是否已经领取
};
...
enum EventTargetType  //被其他地方调用的钩子 必须与玩家相关的数据
{
	...
	ET_ToLevel = 10,//到指定等级
	ET_MaxCurren = 11,//最大拥有乐币(砖石)  
	ET_Recharge  = 12,//充值人民币事件
	ET_CompleteDayTask = 13,//完成日任务事件
	ET_CompleteWeekTask = 14,//完成周常任务事件
	ET_Recharge_One = 15,//充值固定档位  目前只是一元档位
	ET_Recharge_First = 16,//首充事件
	ET_Max,
};
...
struct tagTaskConfig
{
	...
	BYTE		Sign; //1是日常  2是周常  3是日常活跃度  4是周常活跃度
	WORD        Activeness;//活跃度
...
struct tagTaskMap
{
	...
	HashMap< BYTE, std::vector<BYTE> > m_TaskSignMems;
};
//成就配置
...
enum ShopItemType
{
	SIT_Normal = 0,//普通物品
	SIT_Entity = 1,//实体物品 
	SIT_PhonePay = 2,//手机话费  手机话费现在做成实物
...
struct tagRoleGameData
{
	...
	__int64			RoleGetGlobelSum;//玩家获得总金币数
...
struct tagQueryDetailRoleInfo
{
	...
	DWORD			ClientIP;//玩家IP
	bool			IsShowIpAddress;
	TCHAR			IPAddress[MAX_ADDRESS_LENGTH_IP + 1];//IP地址
	BYTE			VipLevel;
	bool			IsInMonthCard;
	DWORD			GameID;
	DWORD           dwCashpoint;       //点券
...
struct tagFishSystemInfo//系统信息
{
	...
	__int64		MaxGobelSum;//42Y 玩家最大金币上限
	...
	WORD        EmailCashpointRewardID;
	DWORD       GoldBulletItemID;
	WORD        MaxSendRecvItemRecord;
	WORD        NewFishRewardID;
	WORD        ServerState;
	WORD        LogonServerCheck;
};
enum RechargeType
{
	RT_Cashpoint = 1,// 充值点券
...
struct OpenLevelMap
{
	HashMap<DWORD, WORD>  m_OpenLevelIDMap;
};

//LevelReward
struct OpenLevelIPMap
{
	HashMap<DWORD, WORD>  m_OpenLevelIPMap;
};
...
struct tagVipConfig
{
	...
	DWORD GetSendGoldBulletNum(BYTE VipLevel)
	{
		HashMap<BYTE, tagVipOnce>::iterator Iter = VipMap.find(VipLevel);
		if (Iter == VipMap.end())
		{
		...
		return Iter->second.SendGoldBulletNum;
	}

	DWORD GetSendSilverBulletNum(BYTE VipLevel)
	{
		HashMap<BYTE, tagVipOnce>::iterator Iter = VipMap.find(VipLevel);
		if (Iter == VipMap.end())
		{
		...
		return Iter->second.SendSilverBulletNum;
	}


	DWORD GetSendBronzeBulletNum(BYTE VipLevel)
	{
		HashMap<BYTE, tagVipOnce>::iterator Iter = VipMap.find(VipLevel);
		if (Iter == VipMap.end())
		{
		...
		return Iter->second.SendBronzeBulletNum;
	}

	DWORD GetSendItemNum(BYTE VipLevel)
	{
		HashMap<BYTE, tagVipOnce>::iterator Iter = VipMap.find(VipLevel);
		if (Iter == VipMap.end())
		{
		...
		return Iter->second.SendItemNum;
	}
...
struct tagFishDropOnce
{
	...
	//vector<DropInfo> QianPaoDroupVec;
	...
	//WORD GetFishQianPaoDrop()
	//{
	//	if (QianPaoDroupVec.empty())
	//		return 0;
	//	DWORD RandValue = RandUInt() % TotalRateValue;
	//	vector<DropInfo>::iterator Iter = QianPaoDroupVec.begin();
	//	//WORD UpperRewardID = 0;
	//	for (; Iter != QianPaoDroupVec.end(); ++Iter)
	//	{
	//		if (RandValue < Iter->RateValue)
	//		{
	//			return Iter->RewardID;
...
{
	HashMap<BYTE, std::map<BYTE, tagFishDropOnce> > FishDropMap;
};
...
enum FishDBCmd
{
	...
	DBR_LoadUserMailRecord = DB_Main_Init + 2,
	DBO_LoadUserMailRecord = DB_Main_Init + 2,
	...
	DBR_LoadUserSendItem = DB_Main_Init + 7,//好友赠送物品邮件相关
	DBO_LoadUserSendItem = DB_Main_Init + 7,
	DBR_AddAllUserMail = DB_Main_Init + 8,
	DBO_AddAllUserMail = DB_Main_Init + 8,
	//Role
	DB_Role_Init,
	...
	DBR_SaveRoleCashpoint = DB_Role_Init + 44,
	DBR_SaveRoleGoldBulletNum = DB_Role_Init + 45,
	DBR_SaveRoleUsingLauncher = DB_Role_Init + 46,
	DBR_SaveRoleNobilityPoint = DB_Role_Init + 47,
	DBR_SaveRoleAddupCheckNum = DB_Role_Init + 48,
	DBR_SaveRoleDayTaskActiviness = DB_Role_Init + 49,
	DBR_SaveRoleWeekTaskActiviness = DB_Role_Init + 50,
	DBR_SaveRoleWeekGlobeNum = DB_Role_Init + 51,
	DBR_SaveRoleGuideStep  = DB_Role_Init + 52,

	DBR_SaveRoleSendGoldBulletNum = DB_Role_Init + 53,
	DBR_SaveRoleSendSilverBulletNum = DB_Role_Init + 54,
	DBR_SaveRoleSendBronzeBulletNum = DB_Role_Init + 55,
	DBR_SaveRoleLastOnlineTime = DB_Role_Init + 56,
	...
	DBR_Query_RoleInfoByIP = DB_Query_Init + 4,
	DBO_Query_RoleInfoByIP = DB_Query_Init + 4,
	...
	DBR_ClearRoleDayTask = DB_Task_Init + 6,//清空一个玩家日常的任务
	DBR_ClearRoleWeekTask = DB_Task_Init + 7,//清空一个玩家周常的任务
	...
	DBR_AddRoleShopNormalItem = DB_Shop_Init+2,
	...
	DBR_LoadRoleExchangeEntity = DB_Enetity_Init + 9,
	DBO_LoadRoleExchangeEntity = DB_Enetity_Init + 9,
	DBR_LoadRoleExchangeItem = DB_Enetity_Init + 10,
	DBO_LoadRoleExchangeItem = DB_Enetity_Init + 10,
	DBR_SaveRoleEntity = DB_Enetity_Init + 11,
	...
	DBR_LogStockScore = DB_Log_Init + 14,
...
struct DBO_Cmd_LoadUserMailRecord : public NetCmd
{
	DWORD			dwUserID;
	BYTE			States;
	WORD			Sum;
	tagRoleMailRecord     Array[1];
};


struct DBO_Cmd_LoadUserSendItem : public NetCmd
{
	DWORD		dwUserID;
	WORD		Page;
	BYTE		States;
	WORD		Sum;
	tagRoleSendItem Array[1];
};


struct DBR_Cmd_LoadUserSendItem : public NetCmd
{
	DWORD	dwUserID;
	WORD	Page;//索引开始
};
...
struct DBR_Cmd_SaveRoleCashpoint : public NetCmd
{
	DWORD			dwUserID;
	DWORD			dwCashpointNum;
};

struct DBR_Cmd_SaveRoleLastOnlineTime: public NetCmd
{
	DWORD			dwUserID;
	//time_t			lastOnlineTime;
};
struct DBR_Cmd_SaveRoleGoldBulletNum : public NetCmd
{
	DWORD			dwUserID;
	DWORD			dwGoldBulletNum;
};

struct DBR_Cmd_SaveRoleSendGoldBulletNum : public NetCmd
{
	DWORD			dwUserID;
	DWORD			BulletNum;
};
struct DBR_Cmd_SaveRoleSendSilverBulletNum : public NetCmd
{
	DWORD			dwUserID;
	DWORD			BulletNum;
};
struct DBR_Cmd_SaveRoleSendBronzeBulletNum : public NetCmd
{
	DWORD			dwUserID;
	DWORD			BulletNum;
};

struct DBR_Cmd_SaveRoleUsingLauncher : public NetCmd
{
	DWORD			dwUserID;
	BYTE			byUsingLauncher;
};
...
struct DBR_Cmd_SaveRoleNobilityPoint : public NetCmd
{
	DWORD			dwUserID;
	DWORD			Sum;
};
struct DBR_Cmd_SaveRoleAddupCheckNum : public NetCmd
{
	DWORD			dwUserID;
	DWORD CheckData;//7日累计
	BYTE AddupCheckNum;//15日累计
	bool IsCheckToday;//今日是否签
};

struct DBR_Cmd_SaveRoleDayTaskActiviness : public NetCmd
{
	DWORD			dwUserID;
	BYTE			Sum;
};

struct DBR_Cmd_SaveRoleWeekTaskActiviness : public NetCmd
{
	DWORD			dwUserID;
	BYTE			Sum;
};

struct DBR_Cmd_SaveRoleWeekGlobeNum : public NetCmd
{
	DWORD			dwUserID;
	__int64			Num;
};

struct DBR_Cmd_SaveRoleGuideStep : public NetCmd
{
	DWORD			dwUserID;
	BYTE			byStep;
};
...
struct DBR_Cmd_SaveRoleOnlineSec : public NetCmd
{
	DWORD			dwUserID;
	DWORD			OnLineSec;
};
...
//	tagQueryDetailRoleInfo RoleInfo;
//};
...
struct DBR_Cmd_Query_RoleInfoByGameID : public NetCmd
{
	...
	DWORD		dwDestID;
	BYTE        Type;
	bool        IsUserID;//默认是false 为gameid
};
...
struct DBR_Cmd_Query_RoleInfoByIP : public NetCmd
{
	DWORD		dwUserID;
	DWORD	    ClientIP;
	BYTE		Page;//页面数
...
struct DBO_Cmd_Query_RoleInfoByIP : public NetCmd
{
	DWORD		dwUserID;
	WORD		Page;
	BYTE		States;
...
struct DBR_Cmd_ClearRoleDayTask : public NetCmd
{
	DWORD	dwUserID;
	//BYTE    byMaxDayTaskID;
	BYTE    Sign;
...
struct DBR_Cmd_ClearRoleWeekTask : public NetCmd
{
	DWORD	dwUserID;
	//BYTE    byMaxDayTaskID;
	BYTE    Sign;
...
struct DBR_Cmd_AddRoleShopNormalItem : public NetCmd
{
	DWORD	dwUserID;
	DWORD	ItemID;
	DWORD	ItemSum;
...
struct DBR_Cmd_LoadRoleExchangeEntity : public NetCmd
{
	DWORD	dwUserID;
	WORD	Page;//总数
};
...
struct DBR_Cmd_LoadRoleExchangeItem : public NetCmd
{
	DWORD	dwUserID;
	WORD	Page;//总数
};

struct DBO_Cmd_LoadRoleExchangeEntity : public NetCmd
{
	DWORD		dwUserID;
	WORD		Page;
	BYTE		States;
	WORD		Sum;
	tagRoleExchangeInfo Array[1];
};


struct DBO_Cmd_LoadRoleExchangeItem : public NetCmd
{
	DWORD		dwUserID;
	WORD		Page;
	BYTE		States;
...
struct DBR_Cmd_LogItemInfo :public NetCmd
{
	DWORD			UserID;
	BYTE			Type;
	int				TypeSum;
...
struct DBR_Cmd_LoadRoleSendItem : public NetCmd
{
	DWORD	dwUserID;
	BYTE	Page;//页面数
};
...
struct DBO_Cmd_LoadRoleSendItem : public NetCmd
{
	DWORD		dwUserID;
	WORD		Page;
	BYTE		States;
...
//char
#define MAX_ROLECHAR_LENGTH 256
...
struct LO_Cmd_ChannelLogon : public NetCmd
{
	...
	//易接传入：
//参数名  类型  必须  示例  说明
//sdk  String  是  09CE2B99C22E6D06  渠道在易接服务器上的ID。由易接提供。
//app  String  是  800018D72E2761D0  CP游戏在易接服务器上的ID。由易接提供。
//uin  String  是  123456  在渠道上的用户ID。
//sess  String  是  abcdefghijklmn  用户在登陆渠道后，由渠道返回给客户端的SessionId。
};
...
enum TableCmd
{
	...
	LC_TableChangeCurrency = 22,
	LC_ResetOtherUserInfo = 30,
	LC_LeaveTableByServer = 31,//服务器让玩家离开桌子
	LC_TableChangeRoleMaxRate = 32,//服务器让玩家离开桌子
};
...
struct LC_Cmd_TableChangeRoleMaxRate : public NetCmd
{
	DWORD		dwDestUserID;
	BYTE 		byMaxRate;
};

struct LC_Cmd_TableChangeRoleCurrency : public NetCmd
{
	DWORD		dwDestUserID;
	__int32		dwCurrencySum;
};
...
enum ItemSub
{
	...
	CL_OnAcceptItem = 9,//赠送物品给好友
	LC_OnAcceptItem = 10,
	CL_LoadUserSendItem = 11,
	LC_LoadUserSendItem = 12,
};
...
struct CL_Cmd_LoadUserSendItem : public NetCmd
{
	BYTE		Page;//页面数 暂时不用 后端控制 
};

struct LC_Cmd_LoadUserSendItem : public NetCmd
{
	WORD		Page;
	BYTE		States;
	WORD		Sum;
...
enum RelationSub
{
	...
	CC_ChangeRelationWeekGlobal = 45,
	LC_ChangeRelationWeekGlobal = 46,
};
...
struct CC_Cmd_ChangeRelationWeekGlobal : public NetCmd
{
	DWORD  dwUserID;
	DWORD  dwDestUserID;
	__int64   WeekGlodNum;
...
struct LC_Cmd_ChangeRelationWeekGlobal : public NetCmd
{
	DWORD  dwDestUserID;
	__int64   WeekGoldNum;
};
...
enum MailSub //系统邮件应该是由中央服务器进行发送的 然后同步到GameServer去
{
	...
	CL_GetAllMailItem = 29,
	LC_GetAllMailItem = 30,
};
...
struct LC_Cmd_NormalMail : public NetCmd
{
	BYTE				States;
	WORD				Sum;
	tagNormalMail	Array[1];
};
struct LC_Cmd_SystemMail : public NetCmd
{
	BYTE				States;
	WORD				Sum;
	tagSystemMail  Array[1];
};
...
struct CL_Cmd_GetAllMailItem : public NetCmd
{
	BYTE 				Type;//0系统 1为好友邮件
};

struct LC_Cmd_GetAllMailItem : public NetCmd
{
	BYTE 				Type;//0系统 1为好友邮件
	bool				Result;
};
...
enum RoleSub
{
	...
	OL_UpdateAccount			= 88,
		...
		//属性变化
	LC_ChangeRoleNobilityPoint = 89,
	LC_ChangeRoleAddupCheckNum = 90,

	LC_ChangeRoleDayTaskActiviness = 91,
	LC_ChangeRoleWeekTaskActiviness = 92,
	LC_ChangeRoleWeekGlobeNum = 93,
	LC_ChangeRoleOnlineRewardStates = 94,
    LC_ChangeRoleTotalTaskStates = 95,
	CL_ChangeRoleGuideStep= 96,
	CC_ChangeRoleWeekGlobe = 97,
	LC_ChangeSendGoldBulletNum = 98,
	LC_ChangeSendSilverBulletNum = 99,
	LC_ChangeSendBronzeBulletNum = 100,
};
...
struct LC_Cmd_ChangeRoleCashpoint : public NetCmd
{
	DWORD		dwCashpointNum;
};

struct LC_Cmd_ChangeRoleSendGoldBulletNum : public NetCmd
{
	DWORD		Num;
};
struct LC_Cmd_ChangeRoleSendSilverBulletNum : public NetCmd
{
	DWORD		Num;
};
struct LC_Cmd_ChangeRoleSendBronzeBulletNum : public NetCmd
{
	DWORD		Num;
};
...
struct LC_Cmd_ChangeRoleTotalTaskStates : public NetCmd
{
	int256	States;
};
...
struct LC_Cmd_ChangeRoleOnlineRewardStates : public NetCmd
{
	//bool 	bComplete;
	DWORD	States;
	DWORD   OnlineSec;
...
struct CL_Cmd_ChangeRoleGuideStep : public NetCmd
{
	BYTE		byStep;
};
...
struct LC_Cmd_ChangeRoleNobilityPoint : public NetCmd
{
	DWORD			Sum;
};
struct LC_Cmd_ChangeRoleAddupCheckNum : public NetCmd
{
	DWORD CheckData;//7日累计
	BYTE AddupCheckNum;//15日累计
	bool IsCheckToday;//今日是否签
...
struct LC_Cmd_ChangeRoleDayTaskActiviness : public NetCmd
{
	BYTE			Sum;
};

struct LC_Cmd_ChangeRoleWeekTaskActiviness : public NetCmd
{
	BYTE			Sum;
};

struct LC_Cmd_ChangeRoleWeekGlobeNum : public NetCmd
{
	__int64		Num;
};
...
struct CC_Cmd_ChangeRoleWeekGlobal : public NetCmd
{
	DWORD  dwUserID;
	__int64  WeekGoldNum;
};
...
struct CL_Cmd_ForgeChangeRateValue : public NetCmd
{
	BYTE		RateIndex;//需要解锁的倍率
	//DWORD		ItemID;
	DWORD       ItemNum;//原石数量
...
struct LC_Cmd_ForgeChangeRateValue : public NetCmd
{
	bool        Result;//参数错误系统错误 
	bool        Success;//锻造成功失败  概率性的
	BYTE		RateIndex;//需要解锁的倍率   当前最大倍率索引  无变化表示失败
};

struct CL_Cmd_DeCompose : public NetCmd
{
	DWORD		ItemID;
	DWORD       ItemNum;
};

struct LC_Cmd_DeCompose : public NetCmd
{
	bool        Result;
	DWORD		ItemID;
	DWORD       ItemNum;
...
struct CL_Cmd_Compose : public NetCmd
{
	DWORD		ItemID;
	DWORD       ItemNum;
};

struct LC_Cmd_Compose : public NetCmd
{
	bool        Result;
	DWORD		ItemID;
	DWORD       ItemNum;
...
enum QuerySub
{
	...
	CL_QueryUserInfoByIP     = 4,//寻找ip
	LC_QueryUserInfo = 10,
	LC_QueryUserInfoByGameID = 13,
	//LC_QueryUserInfoFinish = 11,

	LC_QueryUserOnce = 12,
	LC_QueryUserInfoByIP = 14,
	CL_QueryOneUserDetailInfo = 15,
	LC_QueryOneUserDetailInfo = 16,
...
{
	DWORD		GameID;
	BYTE        Type;//0是名片  1是查找信息
	bool        IsUserID;//默认是gameid
};

struct CL_Cmd_QueryUserInfoByIP : public NetCmd
{
	BYTE		Page;//页面数
	bool		IsOnline;//是否查询不在线的玩家 (可以进行控制的)
};
...
struct LC_Cmd_QueryUserInfoByGameID : public NetCmd
{
	BYTE		Success;//0查找成功
	tagQueryRoleInfoClient ClientInfo;
};

struct LC_Cmd_QueryUserInfoByIP : public NetCmd
{
	WORD		Page;
	BYTE		States;
	WORD		Sum;
...
struct LC_Cmd_QueryUserOnce : public NetCmd
{
	...
	BYTE   Type;
	tagQueryDetailRoleInfo   DetailRoleInfo;
};
...
enum MonthSub
{
	...
	CG_MonthStates = 50,
	CL_ChangeLauncherType = 51,
};

struct LC_ChangeLauncherType : public NetCmd
{
	BYTE LauncherType;
};
...
enum EntitySub
{
	...
	CL_LoadUserExchangeEntity = 21,
	LC_LoadUserExchangeEntity = 22,

	CL_LoadUserExchangeItem = 23,
	LC_LoadUserExchangeItem = 24,
	CL_ChangeEntityInfo = 25,
};
...
struct CL_Cmd_ChangeUserEntity : public NetCmd
{
	tagRoleAddressInfo		UserEntityInfo;
};
...
struct CL_Cmd_LoadUserExchangeEntity : public NetCmd
{
	WORD Page;
	BYTE States;
	WORD Sum;
...
struct CL_Cmd_LoadRoleExchangeEntity : public NetCmd
{
	BYTE		Page;//页面数 暂时不用 后端控制 
};

struct LC_Cmd_LoadRoleExchangeEntity : public NetCmd
{
	WORD		Page;
	BYTE		States;
	WORD		Sum;
...
struct CL_Cmd_LoadRoleExchangeItem : public NetCmd
{
	BYTE		Page;//页面数 暂时不用 后端控制
};

struct LC_Cmd_LoadRoleExchangeItem : public NetCmd
{
	WORD		Page;
	BYTE		States;
	WORD		Sum;
...
struct CL_Cmd_NoticeOnlineRewardComplete : public NetCmd
{
	BYTE		ID;
};
...
struct LC_Cmd_LauncherType : public NetCmd
{
	BYTE	LauncherType;//当前炮台
};
...
{
	NT_Normal = 0,//普通公告
	NT_VipLogon = 1,//土豪登录 土豪来了！
	NT_First_Charge = 2,//   小充大赚！
	NT_ChangeVip = 3,//   众人膜拜！
	NT_DropGold = 4,//   暴富啦！恭
	NT_DropBullet = 5,//   太棒了！恭
	NT_Exchange = 6,//   太牛了！恭
};
...
{
	unsigned __int64  PhoneNumber;//手机号
	WORD            Zone;//区号
	DWORD			BindNumber;//验证码
	DWORD			SecPasswordCrc1;
	DWORD			SecPasswordCrc2;
...
enum ControlSub
{
	...
	CL_HandleEntityItem = 10,//实体物品发货 //线上需要。目前版本不需要
