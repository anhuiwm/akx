...
bool NewGateServer::RecvDataByUDPServer(GWClientData *pc, char *Buff, int nSize, UINT tick)
{
	...
	while (pBuff < pEnd)
	{
		...
		else
		{
			...
			{
				Log("命令的长度不正确2:%d, %d, %d,nSize=%d,cmdtype=%d", pcmd->GetCmdSize(), m_InitData.BuffSize, pEnd - pBuff, nSize, pcmd->GetCmdType());
...
bool NewGateServer::RecvDataByUDP(GWClientData *pc, char *Buff, int nSize, UINT tick)
{
	...
	while (pBuff < pEnd)
	{
		...
		else
		{
			...
			if (pcmd->GetCmdSize() > m_InitData.BuffSize || pcmd->GetCmdSize() > pEnd - pBuff)
			{
				Log("命令的长度不正确3:%d, %d, %d,nSize=%d,cmdtype=%d", pcmd->GetCmdSize(), m_InitData.BuffSize, pEnd - pBuff, nSize, pcmd->GetCmdType());
				pc->RemoveCode = REMOVE_CMD_SIZE_ERROR;
				pc->Removed = true;
				return false;
			}
